...
$id_f=$_GET['id_franq'];
	...
	if($id_f!='0'){
		...
		$titulo_analisis="RESUMEN DE CARTERA POR SECTOR DE $nombre_franq";
	}
	else{
		$titulo_analisis='RESUMEN DE CARTERA POR SECTOR GENERAL';
	}
...
class PDF extends FPDF
{
	...
	function Cuerpo($acceso,$id_f)
	{
		...
		$this->Cell($right,6,strtoupper(_('deuda de abonados por SECTOR ')),"0",0,"L");
		$this->Ln();
		...
		$dato=lectura($acceso,"SELECT * FROM vista_sector1 WHERE id_sector<>'' $consult ");
		for($j=0;$j<count($dato);$j++){
			$cs=0;
			$nombrestatus=trim($dato[$j]["nombre_sector"]);
			$id_sector=trim($dato[$j]["id_sector"]);
			$abrev=strtoupper(trim($dato[$j]["abrev"]));
			//echo "<br>select count(*) as cont from contrato,vista_ubica where contrato.id_calle=vista_ubica.id_calle $consult and contrato.status_contrato='ACTIVO'  and id_sector='$id_sector'<br>";
			$acceso->objeto->ejecutarSql("select count(*) as cont from contrato,vista_ubica where contrato.id_calle=vista_ubica.id_calle $consult and contrato.status_contrato='ACTIVO'  and id_sector='$id_sector'");
			$row=row($acceso);
			$total_abo=trim($row["cont"])+0;
		
			$acceso->objeto->ejecutarSql("select sum(costo_cobro*cant_serv) as monto from contrato,vista_ubica,contrato_servicio_deuda where contrato.id_calle=vista_ubica.id_calle and contrato_servicio_deuda.id_contrato=contrato.id_contrato  $consult and contrato.status_contrato='ACTIVO'  and id_sector='$id_sector'  and fecha_inst < '$fecha_actu'");
			$row=row($acceso);
			$monto_total=trim($row["monto"]);
			
			

			$cable->objeto->ejecutarSql("select count(*) as cont from contrato,vista_ubica where contrato.id_calle=vista_ubica.id_calle  $consult and contrato.status_contrato='ACTIVO' and id_sector='$id_sector' and (select sum(costo_cobro * cant_serv) from contrato_servicio_deuda where contrato_servicio_deuda.id_contrato=contrato.id_contrato and fecha_inst < '$fecha_act')>0");
			$row1=row($cable);
			...
			$this->Cell($w[1],5,substr("$nombrestatus",0,18),"0",0,'L',$fill);
			...
			while(comparaFecha($fecha,$fecha_ant)>0){
				...
				$monto=0;
				$cant=0;
			/*	echo "<br><br>select nro_contrato,contrato.id_contrato from contrato,vista_ubica where contrato.id_calle=vista_ubica.id_calle  $consult and  id_sector='$id_sector' and 
	...
	(select count(*) from contrato_servicio_deuda where contrato_servicio_deuda.id_contrato=contrato.id_contrato and fecha_inst < '$fecha_ini'  and costo_cobro>0 )<=0 ";*/
				...
				$cable->objeto->ejecutarSql("select nro_contrato,contrato.id_contrato from contrato,vista_ubica where contrato.id_calle=vista_ubica.id_calle  $consult and  status_contrato='ACTIVO'  and id_sector='$id_sector' and 
	(select count(*) from contrato_servicio_deuda where contrato_servicio_deuda.id_contrato=contrato.id_contrato and fecha_inst < '$fecha'  and costo_cobro>0 )>0 and 
	(select count(*) from contrato_servicio_deuda where contrato_servicio_deuda.id_contrato=contrato.id_contrato and fecha_inst < '$fecha_ini'  and costo_cobro>0 )<=0 ");		
			...
			$cant=0;
/*
			echo "<br><br>select nro_contrato,contrato.id_contrato from contrato,vista_ubica where contrato.id_calle=vista_ubica.id_calle $consult  and contrato.status_contrato='ACTIVO' and  id_sector='$id_sector' and 
...
*/
			$cable->objeto->ejecutarSql("select nro_contrato,contrato.id_contrato from contrato,vista_ubica where contrato.id_calle=vista_ubica.id_calle and  status_contrato='ACTIVO'  $consult  and  id_sector='$id_sector' and 
(select count(*) from contrato_servicio_deuda where contrato_servicio_deuda.id_contrato=contrato.id_contrato and fecha_inst < '$fecha'  and costo_cobro>0 )>0 ");		
			while ($row1=row($cable)){
				$cant++;
				$id_contrato=trim($row1['id_contrato']);
