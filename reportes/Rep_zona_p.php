...
$x = new EyeDataGrid($db);
	...
	if($id_f!='0'){
		...
		$ac='';
				$dato=lectura($acceso,"SELECT * FROM statuscont WHERE  status='ACTIVO' AND penet='TRUE' order by nombrestatus");
...
$where="
select nombre_zona,nombre_franq, $ac $total, nro_zona from vista_zona1 $consult_w
";
echo ":$where:";
$x->setQuery("id_zona,nombre_franq","from vista_zona1","","");
$x->consultas($where);

$x->setColumnHeader("nro_zona", _("T. Hogar"));
