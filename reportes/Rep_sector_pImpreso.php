...
class PDF extends FPDF
{
	...
	function Titulo()
	{
		...
		$this->MultiCell(190,7,strtoupper(_('informe de penetracion por sector / status')),'0','C');
	...
	if($id_zona!='' && $id_zona!='0'){
		...
		$this->Cell(30,7,strtoupper(_('nombre zona')),1,0,'J',1);
				...
				for($j=0;$j<count($dato);$j++){
					...
					if($tipo_sta!="INCREMENTA" && $entro==false){
						$ac=$ac." (select count(*) from vista_ubica,contrato,statuscont where contrato.status_contrato=statuscont.nombrestatus and vista_ubica.id_calle=contrato.id_calle and  vista_ubica.id_sector=vista_sector1.id_sector and tipo_sta='INCREMENTA' $consult) as total_inc ,";
						$this->Cell(13,7,strtoupper(_('Total I.')),1,0,'J',1);
						$this->Cell(13,7,"%",1,0,'C',1);
						$entro=true;
					}
					...
					$this->Cell(13,7,strtoupper($abrev),1,0,'C',1);
					$this->Cell(13,7,"%",1,0,'C',1);
				}
...
$where="
select nombre_sector,nombre_zona,nro_sector, $ac $total from vista_sector1 WHERE id_sector<>'' $consult
";
	...
	//	echo ":$where:";
		...
		while ($row=row($acceso))
		{
			...
			$hogar=trim($row["nro_sector"]);

			$this->SetX(10);
			$this->SetFont('Arial','',8);
			$this->Cell(40,6,substr(utf8_d(trim($row["nombre_sector"])),0,20),"LR",0,"J",$fill);
			$this->Cell(30,6,substr(utf8_d(trim($row["nombre_zona"])),0,15),"LR",0,"J",$fill);
				...
				for($j=0;$j<count($dato);$j++){
					...
					if($tipo_sta!="INCREMENTA" && $entro==false){
						...
						$this->Cell(13,6, utf8_d(trim($row["$abrev"])),"LR",0,"C",$fill);
						$this->Cell(13,6, number_format($poc_acti+0, 2, ',', '.'),"LR",0,"C",$fill);
						$entro=true;
					}
			...
			$this->Cell(18,6, utf8_d(trim($row["nro_sector"])),"LR",0,"C",$fill);
			$this->Ln();
			$fill=!$fill;
			$cont++;
		}
			...
			$hogar=$cont_h;
				...
				for($j=0;$j<count($dato);$j++){
					...
					if($tipo_sta!="INCREMENTA" && $entro==false){
						...
						$this->Cell(13,6, $ACTI,"LR",0,"C",$fill);
						$this->Cell(13,6, number_format($poc_acti+0, 2, ',', '.'),"LR",0,"C",$fill);
						$entro=true;
					}
