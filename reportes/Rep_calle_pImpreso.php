...
class PDF extends FPDF
{
	...
	function Titulo()
	{
		...
		$this->MultiCell(190,7,strtoupper(_('informe de penetracion por calle / status')),'0','C');
	...
	$consult='';
		...
		$this->Cell(30,7,strtoupper(_('nombre sector')),1,0,'J',1);
				...
				for($j=0;$j<count($dato);$j++){
					...
					if($tipo_sta!="INCREMENTA" && $entro==false){
						$ac=$ac." (select count(*) from vista_ubica,contrato,statuscont where contrato.status_contrato=statuscont.nombrestatus and vista_ubica.id_calle=contrato.id_calle and  vista_ubica.id_calle=vista_calle1.id_calle and tipo_sta='INCREMENTA' $consult) as total_inc ,";
...
$where="
select nombre_calle,nombre_sector,nro_calle, $ac $total from vista_calle1 where id_calle<>'' $consult order by nombre_sector, nombre_calle
";
	...
	//	echo ":$where:";
		...
		while ($row=row($acceso))
		{
			...
			$hogar=trim($row["nro_calle"]);

			$this->SetX(15);
			$this->SetFont('Arial','',8);
			$this->Cell(40,6,substr(utf8_d(trim($row["nombre_calle"])),0,20),"LR",0,"J",$fill);
			$this->Cell(30,6,substr(utf8_d(trim($row["nombre_sector"])),0,15),"LR",0,"J",$fill);
			$entro=false;
				for($j=0;$j<count($dato);$j++){
					$tipo_sta=trim($dato[$j]["tipo_sta"]);
					if($tipo_sta!="INCREMENTA" && $entro==false){
			...
			$this->Cell(18,6, utf8_d(trim($row["nro_calle"])),"LR",0,"C",$fill);
			$this->Ln();
			$fill=!$fill;
			$cont++;
		}
