...
$x->setQuery("id_contrato,inicial_doc,fecha_pago,nro_factura,cedulacli,(nombrecli || ' ' || apellidocli) as nombrecli,monto_pago,monto_pago as base,monto_pago as iva,status_pago,tipo_cliente","vista_pago_cont","","fecha_pago between '$desde' and '$hasta'");
...
$acceso->objeto->ejecutarSql("SELECT *FROM parametros where id_param='2'");
		...
		$acceso->objeto->ejecutarSql("select sum(monto_pago) as total_p from pagos where status_pago='PAGADO' and fecha_pago between '$desde' and '$hasta'");
		$row=row($acceso);
