...
class PDF extends FPDF
{
	...
	function TituloCampos()
	{
		...
		$w=array(30,65,25,25,25,25);
		$header=array(strtoupper(_('nro franquicia')),strtoupper(_('franquicia')),strtoupper(_('deuda')),strtoupper(_('pagado')),strtoupper(_('total cli')),strtoupper(_('con deuda')));
	...
	function Cuerpo($acceso)
	{
		...
		*/
		...
		while ($row=row($acceso))
		{
			...
			$this->Cell($w[0],6,utf8_d(trim($row["id_franq"])),"LR",0,"J",$fill);
			$this->Cell($w[1],6,utf8_d(trim($row["nombre_franq"])),"LR",0,"J",$fill);
			$this->Cell($w[2],6,number_format(trim($row["deuda"])+0, 2, ',', '.'),"LR",0,"J",$fill);
			$this->Cell($w[3],6,number_format(trim($row["pagado"])+0, 2, ',', '.'),"LR",0,"J",$fill);
			$this->Cell($w[4],6,utf8_d(trim($row["t_cli"])),"LR",0,"J",$fill);
			$this->Cell($w[5],6,utf8_d(trim($row["num_deuda"])),"LR",0,"J",$fill);
