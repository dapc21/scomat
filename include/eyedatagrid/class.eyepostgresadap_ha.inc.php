<?php
	...
	global $clave;
	global $data_base_ha;
	$db = new EyePostgresAdap($host,$usuario,$clave,$data_base_ha);
...
class EyePostgresAdap
{
	...
	{
		//echo "dbname=$this->db_name host=$this->host user=$this->user password=$this->pass<br>";
		if ($persist)
			$link = pg_connect("dbname=$this->db_name host=$this->host  port=5432 user=$this->user password=$this->pass");
		else
			$link = pg_connect("dbname=$this->db_name host=$this->host port=5432 user=$this->user password=$this->pass");
	...
	public function contar($query)
	{
		...
		$valor=explode("     LIMIT",$query);
		$query=$valor[0];
		//echo "<br><br>$query";
		$resultado = pg_query($this->link,$query);
		$this->rows_count=pg_num_rows($resultado);
	}
	public function contar_reg($query)
	{
	//	echo "<br><br>contador:$query<br>";
		$resultado = pg_query($this->link,$query);
		$row=pg_fetch_array($resultado);
		$this->rows_count=trim($row[0])+0;
		/*if($this->rows_count0){
			$this->rows_count=10000;
		}
		*/
	}
	/**
	...
	{
		$this->resCalc($query);
		return pg_field_name($query, $offset);
	}
	
	public function fieldType($query = false, $offset)
	{
		$this->resCalc($query);
		return pg_field_type($query, $offset);
	}
	...
	public function fieldTypeArray($query = false)
	{
		$names = array();
    	...
    	for ( $i = 0; $i < $field; $i++ )
			$names[$this->fieldName($query, $i)] = $this->fieldType($query, $i);
	...
	{
		return pg_real_escape_string($str, $this->link);
	}
	...
	{
		$this->resCalc($result);
		return (int) pg_num_fields($result);
	}
	...
	{
		return (int) pg_insert_id($this->link);
	}
	...
	{
		return (int) pg_affected_rows($this->link);
	}
