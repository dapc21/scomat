...
class EyeDataGrid
{
	...
	public $filter = false; // Current filter
	...
	private $ocultar_pie = ''; // Path to images
	...
	public $img_confirmar = 'confirmar.jpg';
	public $img_negar = 'negar.jpg';
	...
	const STDCTRL_CONFIRMAR = 13;
	const STDCTRL_NEGAR = 14;
	...
	const TYPE_MONTO = 11;
	const TYPE_COLOR = 16;
	const TYPE_COLOR_TERM = 17;
	const TYPE_BACK_COLOR = 18;
	const TYPE_DOC = 15;
	...
	*/
	public function __construct(EyePostgresAdap $_db, $image_path = '')
	{
		...
		$this->metodo=(isset($_GET['metodo'])) ? $_GET['metodo'] : 'listar_datos_general';
		$this->tblpage=trim($_GET['page']);
		$this->tblorder=trim($_GET['order']);
		$this->tblfilter=trim($_GET['filter']);
		$this->tblresul=trim($_GET['tblresul']);
	...
	public function ocultar_foot($valor)
	{
		$this->ocultar_pie=$valor;
	}
	...
	public function addStandardControl($type, $action, $action_type = self::TYPE_ONCLICK)
	{
		...
		switch ($type)
		{
			case self::STDCTRL_VDATOS:
				$this->controls[] = '&nbsp;<a ' . $action . '><button type="button" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="bottom" title="Ver Datos"><i class="fa fa-table"></i></button></a>';
				break;
			case self::STDCTRL_REFRES:
				$this->controls[] = '&nbsp;<a ' . $action . '><button type="button" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="bottom" title="Refrescar"><i class="glyphicon glyphicon-repeat"></i></i></button></a>';
				break;
			case self::STDCTRL_EDIT:
				$this->controls[] = '&nbsp;<a ' . $action . '><button type="button" class="btn btn-primary btn-xs" data-toggle="tooltip" data-placement="bottom" title="Editar"><i class="fa fa-pencil"></i></button></a>';
				break;
			case self::STDCTRL_REC:
				$this->controls[] = '&nbsp;<a ' . $action . '><button type="button" class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="bottom" title="Rechazar"><i class="fa fa-thumbs-o-down"></i></button></a>';
				break;
			case self::STDCTRL_ASIG:
				$this->controls[] = '&nbsp;<a ' . $action . '><button type="button" class="btn btn-warning btn-xs" data-toggle="tooltip" data-placement="bottom" title="Asignar Falla"><i class="fa fa-bullhorn"></i></button></a>';
				break;
			case self::STDCTRL_VISITA:
				$this->controls[] = '&nbsp;<a ' . $action . '><button type="button" class="btn btn-primary btn-xs" data-toggle="tooltip" data-placement="bottom" title="Agregar Visita"><i class="fa fa-share-square-o"></i></button></a>';
				break;
			case self::STDCTRL_SAVE:
				$this->controls[] = '&nbsp;<a ' . $action . '><button type="button" class="btn btn-success btn-xs" data-toggle="tooltip" data-placement="bottom" title="Guardar"><i class="fa fa-save "></i></button></a>';
				break;
			/*case self::STDCTRL_NEGAR:
				$this->controls[] = '<a ' . $action . '><img src="' . $this->image_path . $this->img_negar . '" alt="Negar" title="'._("Negar").'" class="tbl-control-image"></a>';
				break;*/
			case self::STDCTRL_NEGAR:
				$this->controls[] = '&nbsp;<a ' . $action . '><button type="button" class="btn btn btn-danger btn-xs" data-toggle="tooltip" data-placement="bottom" title="Negar"><i class="glyphicon glyphicon-trash"></i></button></a>';
				break;
			/*case self::STDCTRL_CONFIRMAR:
				$this->controls[] = '<a ' . $action . '><img src="' . $this->image_path . $this->img_confirmar . '" alt="Autorizar" title="'._("Autorizar").'" class="tbl-control-image"></a>';
				break;*/
			case self::STDCTRL_CONFIRMAR:
				$this->controls[] = '&nbsp;<a ' . $action . '><button type="button" class="btn btn-success btn-xs" data-toggle="tooltip" data-placement="bottom" title="Autorizar"><i class="glyphicon glyphicon-ok"></i></button></a>';
				break;
			case self::STDCTRL_PRINT:
				$this->controls[] = '&nbsp;<a ' . $action . '><button type="button" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="bottom" title="Imprimir"><i class="fa fa-print"></i></button></a>';
				break;
			case self::STDCTRL_FINALIZAR:
				$this->controls[] = '&nbsp;<a ' . $action . '><button type="button" class="btn btn-warning btn-xs" data-toggle="tooltip" data-placement="bottom" title="Finalizar"><i class="fa fa-sign-out"></i></button></a>';
				break;
			case self::STDCTRL_DELETE:
				$this->controls[] = '&nbsp;<a ' . $action . '><button type="button" class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="bottom" title="Eliminar"><i class="fa fa-trash-o"></i></button></a>';
				break;
			default:
	...
	*/
	private function parseVariables(array $row, $act)
	{
		...
		foreach($vars[0] as $v)
			$act = str_replace(trim($v), $row[str_replace('%', '', $v)], $act);
	...
	*/
	private function buildHeader()
	{
		...
		/*echo*/ echo '<thead><tr class="titulo-tabla" style="background-color: #344151">';
		...
		{
			$this->column_count++;
			/*echo*/ echo '<th align="center"><a href="javascript:;" align="center"><strong align="center">&nbsp;#&nbsp;</strong></a></th>';
		}
		...
		if ($this->show_checkboxes)
		{
			...
			else if($this->clase=="Nota Credito"){
				echo '<th class="tbl-header-check numeric" align="center" ><input type="checkbox" name="checkbox" onclick="tblToggleCheckAll();colocar_monto_nc();calcularMonto_NC();"> <input  type="hidden" name="email" maxlength="40" size="10" value="" ></th>';
			}
			else if($this->clase=="Nota Debito"){
				echo '<th class="tbl-header-check numeric" align="center" ><input type="checkbox" name="checkbox" onclick="tblToggleCheckAll();colocar_monto_nd();calcularMonto_ND();"> <input  type="hidden" name="email" maxlength="40" size="10" value="" ></th>';
			}
		...
		foreach ($headers as $t)
		{
			...
			if ($this->hide_order){
				...
				if ($this->order and $this->order['Column'] == $t)
					...
					echo '<th><a href="javascript:;" onclick="tblSetOrder(\'' . $t . '\', \'' . $order . '\',\''.$this->archivoDataGrid.'\',\''.$this->divDataGrid.'\',\''.$this->metodo.'\',\''.$this->tblpage.'\',\''.$this->tblorder.'\',\''.$this->tblfilter.'\',\''.$this->tblresul.'\')">' . strtoupper($header) . "</a>";
			...
			if ($this->allow_filters and
				!in_array($this->type[$t][0], array(
									...
									self::TYPE_MONTO,
									self::TYPE_COLOR,
									self::TYPE_COLOR_TERM,
									self::TYPE_BACK_COLOR,
									self::TYPE_DOC,
									self::TYPE_CHECK,
									self::TYPE_CUSTOM,
									self::TYPE_PERCENT
									)))
			{
				...
				/*echo*/ echo ' <a href="javascript:;" onclick="tblShowHideFilter(\'' . $t . '\',\''.trim($this->archivoDataGrid).'\',\''.trim($this->divDataGrid).'\')"> <i class="fa fa-search fa-rotate-90"></i></a><br><div class="tbl-filter-box" id="filter-'.$this->divDataGrid . $t . '" style="display:' . $filter_display . '; font: 8pt Arial;" ><input type="text" size="5" style="font: 8pt Arial;" id="filter-value-' .$this->divDataGrid. $t . '" value="'.$filter_value.'"  onchange="tblSetFilter(\'' . $t . '\',\''.trim($this->archivoDataGrid).'\',\''.trim($this->divDataGrid).'\',\''.$this->metodo.'\',\''.$this->tblpage.'\',\''.$this->tblorder.'\',\''.$this->tblfilter.'\',\''.$this->tblresul.'\')">&nbsp;<a href="javascript:;" onclick="tblSetFilter(\'' . $t . '\',\''.trim($this->archivoDataGrid).'\',\''.trim($this->divDataGrid).'\',\''.$this->metodo.'\',\''.$this->tblpage.'\',\''.$this->tblorder.'\',\''.$this->tblfilter.'\',\''.$this->tblresul.'\')">Filtrar</a></div>';
			}
	...
	private function buildFooter($shown, $first = 0, $last = 0)
	{
		...
		<ul>';
			...
			if ($this->page > 1){
				echo '
				<li class="prev">
				<a href="javascript:;" onclick="tblSetPage(1,\''.$this->archivoDataGrid.'\',\''.$this->divDataGrid.'\',\''.$this->metodo.'\',\''.$this->tblpage.'\',\''.$this->tblorder.'\',\''.$this->tblfilter.'\',\''.$this->tblresul.'\')"><i class="fa fa-angle-double-left"></i></a>
				</li>
				<li class="prev">
				<a href="javascript:;" onclick="tblSetPage(' . ($this->page - 1) . ',\''.$this->archivoDataGrid.'\',\''.$this->divDataGrid.'\',\''.$this->metodo.'\',\''.$this->tblpage.'\',\''.$this->tblorder.'\',\''.$this->tblfilter.'\',\''.$this->tblresul.'\')"><i class="fa fa-angle-left"></i></a>
				</li>';
			}
			else{
				echo '
			...
			for ($i = $startpage; $i <= $endpage; $i++)
			{
				if ($i == $this->page){						
					...
					echo '
					<li>
					<a href="javascript:;" onclick="tblSetPage(' . $i . ',\''.$this->archivoDataGrid.'\',\''.$this->divDataGrid.'\',\''.$this->metodo.'\',\''.$this->tblpage.'\',\''.$this->tblorder.'\',\''.$this->tblfilter.'\',\''.$this->tblresul.'\')">' . $i . '</a>
					</li>';
				}
			}
			...
			if($this->page < $pages){
				echo '
				<li class="next">
				<a href="javascript:;" onclick="tblSetPage(' . ($this->page + 1) . ',\''.$this->archivoDataGrid.'\',\''.$this->divDataGrid.'\',\''.$this->metodo.'\',\''.$this->tblpage.'\',\''.$this->tblorder.'\',\''.$this->tblfilter.'\',\''.$this->tblresul.'\')"><i class="fa fa-angle-right"></i></a>
				</li>
				<li class="next">
				<a href="javascript:;" onclick="tblSetPage(' . $pages . ',\''.$this->archivoDataGrid.'\',\''.$this->divDataGrid.'\',\''.$this->metodo.'\',\''.$this->tblpage.'\',\''.$this->tblorder.'\',\''.$this->tblfilter.'\',\''.$this->tblresul.'\')"><i class="fa fa-angle-double-right"></i></a>
				</li>';
			}
			else{
				echo '
		...
		echo '
			...
			<input class="form-control resultados" type="text" style="width:75px;" maxlength="10" name="tblresul_'.$this->divDataGrid.'" id="tblresul_'.$this->divDataGrid.'" value="'.$this->results_per_page.'" onChange="tblSetResul(\''.$this->archivoDataGrid.'\',\''.$this->divDataGrid.'\',\''.$this->metodo.'\',\''.$this->tblpage.'\',\''.$this->tblorder.'\',\''.$this->tblfilter.'\',\''.$this->tblresul.'\')" >
			</div>
		...
		echo '<button type="button" class="btn btn-success contenido-boton-excel" data-toggle="tooltip" data-placement="bottom" title="Exportar Datos a Excel" onclick="updateTableExcel(\''.$this->archivoDataGrid.'\',\''.$this->divDataGrid.'\',\''.$this->metodo.'\',\''.$this->tblpage.'\',\''.$this->tblorder.'\',\''.$this->tblfilter.'\',\''.$this->tblresul.'\')"><i class="fa fa-file-text"></i> </button>' . ' ';	
		echo '<button type="button" class="btn btn-info contenido-boton-todos" data-toggle="tooltip" data-placement="bottom" title="Mostrar Todos los Registros" onclick="tblSetResulTodo(\''.$this->archivoDataGrid.'\',\''.$this->divDataGrid.'\',\''.$this->metodo.'\',\''.$this->tblpage.'\',\''.$this->tblorder.'\',\''.$this->tblfilter.'\',\''.$this->tblresul.'\')"><i class="glyphicon glyphicon-list"></i> </button>';
		echo '
		</div>
		</div>
	...
	*/
	public function printTable()
	{
		...
		if($this->clase=="Pagos"){
			$order = "ORDER BY tipo_serv , fecha_inst";
		}
		...
		if($this->consulta!=''){
			$query_count = '' ;
			$query = $this->consulta;
			$posi=strpos($query, $this->select_table);
			//echo "<br>pos:".$posi."::";
			$query_count = "select count(*) ".substr($query, $posi);
			//echo "<br>str:".$query_count."::";
		}
		else{
			$query = 'SELECT ' . $this->select_fields . ' FROM ' . $this->select_table . ' ' . $filter;
			$query_count = 'SELECT count(*) FROM ' . $this->select_table . ' ' . $filter;
		}
		//echo "$query";
		...
		$this->_db->contar_reg($query_count);
...
//echo ":".$this->row_count.":";
		...
		if ($this->reset_button)
			/*echo*/ echo '<span class="tbl-reset"><a href="javascript:;" onclick="tblReset()" title="' . $this->reset_button .'"><img src="' . $this->image_path . $this->img_reset . '" class="tbl-reset-image">' . $this->reset_button .'</a></span>';

		/*echo*/ echo '<table class="table table-hover table-condensed">';
		...
		if ($this->row_count == 0)
			/*echo*/ echo '<tr><td colspan="' . $this->column_count . '" class="numeric">' . self::TXT_NORESULTS . '</td></tr>';
		else {
			$i = 0; $first = 0; $last = 0;
			...
			while ($row = $this->_db->fetchAssoc($this->result))
			{
				...
				if ($this->show_row_number)
					/*echo*/ echo '<td class="numeric" align="left">' . $line . '</td>';
				...
				if ($this->show_checkboxes){
					if($this->clase=="Pagos"){
							...
							$descu=0;
						 echo '<td class="numeric" align="center"><input type="checkbox" class="tbl-checkbox" name="checkbox" onchange="calcularMontoPago()" value="'.$row[$this->primary].'=@'.$apagar.'=@'.$descu.'"></td>';
					}
					else if($this->clase=="tarifa_servicio"){
							$id_serv=trim($row[$this->primary]);
						 echo '<td class="numeric" align="center"><input type="checkbox" class="tbl-checkbox" name="checkbox" onchange="" value="'.$id_serv.'" id="'.$id_serv.'"></td>';
					}
					else if($this->clase=="Nota Credito"){
							$cant_serv=trim($row['cant_serv']);
							$costo_cobro=trim($row['costo_cobro']);
							...
							$total_pago=$cant_serv*$costo_cobro;
							$descu=0;
						 echo '<td class="numeric" align="center"><input type="checkbox" class="tbl-checkbox" name="checkbox" onchange="colocar_monto_nc_unico(\''.$row[$this->primary].'\');calcularMonto_NC()" value="'.$row[$this->primary].'"></td>';
					}
					else if($this->clase=="Nota Debito"){
							...
							$total_pago=$cant_serv*$costo_cobro;
							$descu=0;
						 echo '<td class="numeric" align="center"><input type="checkbox" class="tbl-checkbox" name="checkbox" onchange="colocar_monto_nd_unico(\''.$row[$this->primary].'\');calcularMonto_ND()" value="'.$row[$this->primary].'"></td>';
					}
					else if($this->clase=="grupo_ubicacion"){
				...
				foreach ($row as $key => $value)
				{
					...
					if($this->clase=="Convenio" && trim($key)=="fecha_ven"){
							$value='<input type="text" id="fecha_ven_'.$row[$this->primary].'" maxlength="10" size="15" value="" >';
					}
					if($this->clase=="Pagos" || $this->clase=="Actualizar_Pagos"  || $this->clase=="historialpago"  || $this->clase=="anular_pagos" || $this->clase=="Convenio"){
						...
						else if(trim($key)=="apagar"){
						//	echo "entro:$value";
							$value='<input disabled style="color:blue; font-weight:blod; font-size: 10pt"  type="text" id="'.$id_cont_serv.'_total" maxlength="10" size="8" onChange="" value="'.$value.'">';
						}
						...
						//echo "<br>:".trim($key).":";
						if(trim($key)=="id_cont_serv"){
							$id_cont_serv=trim($value);
						}
						if(trim($key)=="tipo_costo"){
							$tipo_costo=trim($value);
						}
						else if(trim($key)=="total"){
						//	echo "entro:$value";
							$value='<input disabled style="color:blue; font-weight:blod; font: 12pt arial" class="form-control" type="text" id="'.$id_cont_serv.'_total" maxlength="10" size="10"onChange="" value="'.$value.'">';
						}
						else if(trim($key)=="apagar"){
							$value='<input style="color:blue; font-weight:blod; font: 12pt arial"  class="form-control" type="text" id="'.$id_cont_serv.'_apagar" maxlength="10" size="10" value="'.$value.'"  onChange="calcularMonto_NC_unico(\''.$id_cont_serv.'\')">';
						}
						else if(trim($key)=="fecha_inst"){
						...
						if(trim($key)=="id_serv"){
							...
							$value='<input data-parsley-id="'.$id_serv.'_tar" data-parsley-type="number" style="color:blue; font-weight:blod; font: 11pt arial"  class="form-control" type="text" id="'.$id_serv.'_tar" maxlength="10" size="10" value=""  onChange="activa_check_tar(\''.$id_serv.'\')"> 
							<ul id="parsley-id-'.$id_serv.'_tar" class="parsley-errors-list"></ul>
							';
						}
					}
					else if($this->clase=="Nota Debito"){
						...
						//	echo "entro:$value";
							...
							$value='<input style="color:blue; font-weight:blod; font: 12pt arial"  class="form-control" type="text" id="'.$id_cont_serv.'_apagar" maxlength="10" size="10" value="'.$value.'"  onChange="calcularMonto_ND_unico(\''.$id_cont_serv.'\')">';
						}
						else if(trim($key)=="fecha_inst"){
					...
					else if($this->clase=="DetalleCobros"){
					/*
						if(trim($key)=="nombrecli" and trim($row['status_pago'])=='ANULADO'){
							$value='';
						}
						else if(trim($key)=="nro_contrato" and trim($row['status_pago'])=='ANULADO'){
							$value='';
						}
						else if(trim($key)=="cedulacli" and trim($row['status_pago'])=='ANULADO'){
							$value='';
						}
						*/
					}
						...
						if(trim($key)=="cedulacli" and $value!=''){
					 ...
					 if(trim($key)=="detalle"){
								...
								$id_pago=$value;
							$cable->objeto->ejecutarSql("select nombre_servicio,id_serv,fecha_inst FROM vista_pago_ser where id_pago='$id_pago' order by fecha_inst");
							$nombre_servicio='';
						...
						switch ($type)
						{
							...
							case self::TYPE_MES:
								list($ano,$mes,$dia)=explode("-",$value);
									...
									$value = formato_mes_com1($mes)." $ano";
								break;
							case self::TYPE_MONTO:
									...
									$value = number_format($value+0, 2, ',', '.');
								break;
							case self::TYPE_BACK_COLOR:
									
									$value = "<span style='background-color:".$row['color'].";' >$value </span>";
								break;
							case self::TYPE_COLOR:
									$value = "<span style='color:".$row['color'].";' >$value </span>";
								break;
							case self::TYPE_COLOR_TERM:
									$color = array('ACTIVO' => '#008000','SUSPENDIDO' => '#FF0000','DISPONIBLE' => '#FF0000');
									$value = "<span style='color:".$color[$row['status_es']].";' >$value </span>";
								break;
							case self::TYPE_DOC:
								$val=array("NOTA CREDITO"=>"NC","NOTA DEBITO"=>"ND","PAGO"=>"PAG","FACTURA"=>"FAC","AVISO"=>"AVI","ABONO"=>"ABO");
								$value=trim($value);
								$value = $val["$value"];
								break;
							...
							case self::TYPE_CUSTOM:
								$val=array("t"=>"SI","f"=>"NO",""=>"DISPONIBLE","I"=>"ACTIVO","S"=>"SUSPENDIDO");
								$value=trim($value);
								$value = $val["$value"];
								break;
					...
					/*echo*/ echo '<td class="numeric">' . $value . '</td>';
				}
