...
if($orden_factura_G=='UNICO'){
	...
	$acceso->objeto->ejecutarSql("select nro_factura,inc from pagos,caja_cobrador where  pagos.id_caja_cob=caja_cobrador.id_caja_cob  and tipo_doc='NOTA CREDITO' ORDER BY nro_factura desc LIMIT 1 offset 0 "); 
	$nro_factura = verNumero_factura_v4($acceso,"nro_factura",$dig_fact_G);
}
else if($orden_factura_G=='POR ESTACION'){
	$acceso->objeto->ejecutarSql("select nro_factura,inc from pagos,caja_cobrador where  id_est='$id_est' and pagos.id_caja_cob=caja_cobrador.id_caja_cob and status_pago='PAGADO'  and tipo_doc='NOTA CREDITO' ORDER BY nro_factura desc LIMIT 1 offset 0 "); 
...
if($orden_control_G=='UNICO'){
	$acceso->objeto->ejecutarSql("select nro_control,inc from pagos,caja_cobrador where  pagos.id_caja_cob=caja_cobrador.id_caja_cob  and tipo_doc='NOTA CREDITO' ORDER BY nro_control desc LIMIT 1 offset 0");
	$nro_control = verNumero_control_v4($acceso,"nro_control",$dig_control_G);
}
else if($orden_control_G=='POR ESTACION'){
	$acceso->objeto->ejecutarSql("select nro_control,inc from pagos,caja_cobrador where  id_est='$id_est' and pagos.id_caja_cob=caja_cobrador.id_caja_cob  and tipo_doc='NOTA CREDITO'  ORDER BY nro_control desc LIMIT 1 offset 0");
	$nro_control = verNumero_control_v4($acceso,"nro_control",$dig_control_G);
}
...
<form role="form" name="f1" id="f1" data-parsley-validate="">
	...
	<div class="border-head"><h3>Solicitud de Nota de crédito</h3></div>
	
	<!-- ÁREA DE PANEL O PANELES -->
	<section class="panel">
		
		<!-- CABECERA O TÍTULO DEL PANEL (EQUIVALENTE A UN FIELDSET SIN SERLO) -->
		<header class="panel-heading">Busqueda de  Facturas</header>
		<!-- INICIO DEL PANEL -->
		<div class="panel-body">
			...
			<div class="form-group col-lg-3 col-md-4 col-sm-6 col-xs-12">
				...
				<input data-parsley-id="nro_factura_nc" required="" class="form-control" type="text" name="nro_factura_nc" id="nro_factura_nc" onChange="c_nro_factura_nc();" maxlength="12" size="20" value="" >

				<div class="input-group-btn">
					<button type="button" class="btn btn-info" id="buscar_abonado" onclick="c_nro_factura_nc();"><i class="fa fa-search"></i></button>	
				</div>
				</div>
			...
			<div class="form-group col-lg-3 col-md-4 col-sm-6 col-xs-12">
				...
				<div class="input-group">
				<input data-parsley-id="nro_contrato"  class="form-control" type="text" name="nro_contrato" id="nro_contrato" onChange="c_nro_factura_nc();" maxlength="11" size="12" value="">
				<div class="input-group-btn">
					<button type="button" class="btn btn-info" id="buscar_abonado" onclick="c_nro_factura_nc();"><i class="fa fa-search"></i></button>	
				</div>
				</div>
			...
			<div class="form-group col-lg-3 col-md-4 col-sm-6 col-xs-12">
				...
				<div class="input-group">				
				<input class="form-control" type="text" name="cedula_b" id="cedula_b" maxlength="10" size="15" value="" onChange="c_nro_factura_nc();">
				<div class="input-group-btn">
					<button type="button" class="btn btn-info" id="buscar_rif_cedula" onclick="c_nro_factura_nc();"><i class="fa fa-search"></i></button>	
				</div>
				</div>
			</div>						
	...
	<section class="panel">
		...
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			...
			<div class="form-group col-lg-3 col-md-3 col-sm-6 col-xs-12">
				<label>Total a Nota Credito</label>
				<input  data-parsley-id="monto_pago" required="" class="form-control" type="text" name="monto_pago" id="monto_pago" maxlength="10" size="20" value="0" onchange="adaptar_cargos_nc();" style="color:blue; font-weight:blod;" >
	...
	<section class="panel">
		...
		<div class="form-btn col-lg-12 col-md-12 col-sm-12 col-xs-12" align="center">
			...
			<button class="btn btn-warning" type="<?php echo $obj->mo; ?>" name="nota_credito" id="nota_credito" value="" onclick="gestionar_nota_credito('nota_credito_factura','pagos')"><i class="fa fa-trash-o"></i>realizar nota de credito</button>
			
			<button class="btn btn-success" type="button" name="salir" id="salir" onclick="cargar_form_nota_credito_factura();" value="<?php echo _("limpiar");?>"><i class="glyphicon glyphicon-repeat"></i> Limpiar</button>
		</div>
		</div> <!-- FIN DEL PANEL --> 
	</section> <!-- FIN DEL ÁREA DE PANEL O PANELES -->
