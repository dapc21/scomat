...
function Image($file,$x=NULL,$y=NULL,$w=0,$h=0,$type='',$link='', $isMask=false, $maskImg=0)
{
    ...
    if(!isset($this->images[$file]))
    {
		...
		//comparamos la version de php para determinar cual funcion usaremos 
		//ya que a partir de 5.3.0 está en desuso (deprecated) la manera en como se trataba en versiones inferiores.
...
function Header()
{
	...
	$this->SetXY(45,14);
	$this->MultiCell(220,3,utf8_decode(trim(strtoupper( nombre_empresa() ))),'0','C');
	$this->Ln();
	$this->SetX(45);
	$this->SetFont('Arial','B',10);
	$this->MultiCell(220,3,trim(tipo_serv()),'0','C');
	$this->Ln();
	$this->SetX(45);
	$this->SetFont('Arial','B',10);
	$this->MultiCell(220,3,trim(telef_emp()),'0','C');
	$this->Ln();
}
...
function Titulo($title)
{
	...
	$this->MultiCell(220,3,utf8_decode(trim(strtoupper('Datos Estadísticos de '.$title))),'0','C');
}
