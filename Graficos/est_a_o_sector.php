...
$nombre_zona = trim($row["nombre_zona"]);
	...
	for($j=$inicio_a;$j<=$fin_a;$j++){
		...
		for($i=$mes_ini;$i<=$mes_fin;$i++){
			...
			if($id_det_orden=='TODOS'){
				$corte=verCant($acceso,"select count(ordenes_tecnicos.id_contrato) as total from ordenes_tecnicos,detalle_orden,contrato, calle where ordenes_tecnicos.id_det_orden=detalle_orden.id_det_orden and detalle_orden.id_tipo_orden='$id_tipo_orden' and fecha_orden between '$fec_ini' and '$fec_fin' and calle.id_sector='$id_sector' and ordenes_tecnicos.id_contrato=contrato.id_contrato and contrato.id_calle = calle.id_calle");
			}
			else{
				$corte=verCant($acceso,"select count(ordenes_tecnicos.id_contrato) as total from ordenes_tecnicos,contrato, calle where id_det_orden='$id_det_orden' and fecha_orden between '$fec_ini' and '$fec_fin' and calle.id_sector='$id_sector' and ordenes_tecnicos.id_contrato=contrato.id_contrato and contrato.id_calle = calle.id_calle");
			}
			$total_i += $corte;
			$num_k++;
		}
	...
	$num_k=count($data);
		...
		for($i=0;$i<count($data);$i++){
			...
			$mes_l=$i+1;
			$resumen.='<tr><td width="180px" class="estilocabe"> A&ntilde;o '.$year[$i].'</td><td width="40px" class="estilocabe"  align="center">'.$porcentaje.'%</td<td width="120px" align="right" class="estilocabe">'.number_format($total, 0, ',', '.').'</td></tr>';
		}
...
$title = new title("CANTIDAD DE $nombre_det_orden DE ".strtoupper($nombre_sector));
...
function llamarGrafico(){
	if(parseInt(document.f1.anio_i.value)>parseInt(document.f1.anio_f.value)){
		...
		location.href="est_a_o_sector.php?&tipoGrafico="+document.f1.tipoGrafico.value+"&inicio_a="+document.f1.anio_i.value+"&fin_a="+document.f1.anio_f.value+"&id_det_orden="+document.f1.id_det_orden.value+"&id_tipo_orden="+document.f1.id_tipo_orden.value+"&id_sector="+document.f1.id_sector.value+"&";
	}
}
...
function imprimirGrafico(){
	...
	if(parseInt(document.f1.anio_i.value)>parseInt(document.f1.anio_f.value)){
		...
		location.href="est_a_o_sector_impreso.php?&tipoGrafico="+document.f1.tipoGrafico.value+"&inicio_a="+document.f1.anio_i.value+"&fin_a="+document.f1.anio_f.value+"&id_det_orden="+document.f1.id_det_orden.value+"&id_tipo_orden="+document.f1.id_tipo_orden.value+"&id_sector="+document.f1.id_sector.value+"&";
	}
}
</script>
...
<table border="0" width="760px" align="center"> 
	<tr>
		<td colspan="2" ALIGN="CENTER">
			<div class="cabe">ESTADISTICAS ANUALES DE <?php echo strtoupper(utf8_decode($nombre_det_orden));?> POR SECTOR</div>
		</td>
	</tr>
	<tr>
			<td colspan="2" ALIGN="CENTER" class="estilocabeN">
				<table border="0" width="760px" align="center" cellspacing="2" cellpadding="2"> 
					<tr>
						...
						<td class="estilocabeN" rowspan="3">
					...
					<tr>
						...
						<td>
							<select name="id_sector" id="id_sector" onchange="llamarGrafico()" style="width: 150px;">
