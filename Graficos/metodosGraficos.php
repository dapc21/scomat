...
function ver_anios($acceso){
	$meses = array();
	$acceso->objeto->ejecutarSql("SELECT fecha_pago FROM pagos ORDER BY fecha_pago asc LIMIT 1 offset 0");
	$row=row($acceso);
	$fecha_inicio = trim($row["fecha_pago"]);
	$valor=explode("-",$fecha_inicio);
	$meses[0]=$valor[0];

	$acceso->objeto->ejecutarSql("SELECT fecha_pago FROM pagos ORDER BY fecha_pago desc LIMIT 1 offset 0");
	$row=row($acceso);
	$fecha_fin = trim($row["fecha_pago"]);
	$valor=explode("-",$fecha_fin);
	$meses[1]=$valor[0];
...
function crearIMG($nombre){
$original = imagecreatefrompng("../include/openflashchart/tmp-upload-images/$nombre.png");
...
imagejpeg($thumb,"../include/openflashchart/tmp-upload-images/$nombre.jpg",100); // 90 es la calidad de compresión
}
?>