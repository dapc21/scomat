...
$acceso->objeto->ejecutarSql("SELECT sum(costo_cobro) as deuda FROM contrato_servicio_deuda where fecha_inst between '$fec_ini' and '$fec_fin' and status_con_ser='DEUDA'");
$row=row($acceso);
...
function llamarGrafico(){
	if(parseInt(document.f1.anio_i.value)>parseInt(document.f1.anio_f.value)){
		...
		location.href="est_a_i_fran.php?&tipoGrafico="+document.f1.tipoGrafico.value+"&inicio_a="+document.f1.anio_i.value+"&fin_a="+document.f1.anio_f.value+"&";
	}
}
...
function imprimirGrafico(){
	...
	if(parseInt(document.f1.anio_i.value)>parseInt(document.f1.anio_f.value)){
		...
		location.href="est_a_i_fran_impreso.php?&tipoGrafico="+document.f1.tipoGrafico.value+"&inicio_a="+document.f1.anio_i.value+"&fin_a="+document.f1.anio_f.value+"&";
	}
...
<table border="0" width="760px" align="center"> 
	<tr>
		<td colspan="2" ALIGN="CENTER">
			<div class="cabe">ESTADISTICAS ANUALES DE INGRESOS Y DEUDAS POR FRANQUICIAS</div>
		</td>
	</tr>
	<tr>
