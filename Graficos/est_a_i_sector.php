...
$total_ing = 0;
//echo "SELECT sum(monto_pago) as monto_pago FROM pagos,contrato, calle where fecha_pago between '$fec_ini' and '$fec_fin' and calle.id_sector='$id_sector' and status_pago='PAGADO' and pagos.id_contrato=contrato.id_contrato and contrato.id_calle = calle.id_calle";
...
for($i=$inicio_a;$i<=$fin_a;$i++){
	...
	*/
	...
	$acceso->objeto->ejecutarSql("SELECT sum(costo_cobro) as deuda FROM contrato_servicio_deuda,contrato, calle where fecha_inst between '$fec_ini' and '$fec_fin' and status_con_ser='DEUDA' and calle.id_sector='$id_sector' and contrato_servicio_deuda.id_contrato=contrato.id_contrato and contrato.id_calle = calle.id_calle");
//	echo "<br>SELECT sum(costo_cobro) as deuda FROM contrato_servicio,contrato, calle where fecha_inst between '$fec_ini' and '$fec_fin' and status_con_ser='DEUDA' and sector.id_zona='$id_zona' and contrato_servicio.id_contrato=contrato.id_contrato and contrato.id_calle = calle.id_calle";
	$row=row($acceso);
...
function llamarGrafico(){
	if(parseInt(document.f1.anio_i.value)>parseInt(document.f1.anio_f.value)){
		...
		location.href="est_a_i_sector.php?&tipoGrafico="+document.f1.tipoGrafico.value+"&id_sector="+document.f1.id_sector.value+"&inicio_a="+document.f1.anio_i.value+"&fin_a="+document.f1.anio_f.value+"&";
	}
}
...
function imprimirGrafico(){
	...
	if(parseInt(document.f1.anio_i.value)>parseInt(document.f1.anio_f.value)){
		...
		location.href="est_a_i_sector_impreso.php?&tipoGrafico="+document.f1.tipoGrafico.value+"&id_sector="+document.f1.id_sector.value+"&inicio_a="+document.f1.anio_i.value+"&fin_a="+document.f1.anio_f.value+"&";
	}
...
<table border="0" width="760px" align="center"> 
	<tr>
		<td colspan="2" ALIGN="CENTER">
			<div class="cabe">ESTADISTICAS ANUALES DE INGRESOS Y DEUDAS POR SECTOR</div>
		</td>
	</tr>
	<tr>
