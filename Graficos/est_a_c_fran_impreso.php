...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->MultiCell(190,7,utf8_decode('ESTADISTICAS ANUALES DE CLIENTES ACTIVOS Y CORTADOS POR FRANQUICIAS'),'0','C');
	...
	$num_k=count($data);
		...
		$this->Ln();
		$this->SetX(15);
		$this->Cell(190,6,'RESUMEN DE CLIENTES ACTIVOS Y CORTADOS',"0",0,"C");
		$this->Ln();
		...
		for($i=0;$i<$num_k;$i++){
			...
			$porcentaje1=number_format($porc, 2, ',','.');
				...
				$this->Cell($w[2],6,$total,"1",0,"J");
				$this->Cell($w[3],6,$porcentaje1.'%',"1",0,"J");
				$this->Cell($w[4],6,$deuda,"1",0,"J");
				$this->Ln();
				$cont++;
		}
				$this->SetFont('Arial','B',10);
