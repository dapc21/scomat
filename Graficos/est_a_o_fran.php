...
if($id_det_orden=="TODOS"){
	...
	for($j=$inicio_a;$j<=$fin_a;$j++){
		...
		for($i=$mes_ini;$i<=$mes_fin;$i++){
			...
			if($id_det_orden=='TODOS'){
				$corte=verCant($acceso,"select count(DISTINCT id_contrato)  as total from ordenes_tecnicos,detalle_orden where ordenes_tecnicos.id_det_orden=detalle_orden.id_det_orden and detalle_orden.id_tipo_orden='$id_tipo_orden' and fecha_orden between '$fec_ini' and '$fec_fin'");
			}
			else{
				$corte=verCant($acceso,"select count(DISTINCT id_contrato)  as total from ordenes_tecnicos where id_det_orden='$id_det_orden' and fecha_orden between '$fec_ini' and '$fec_fin'");
			}
			$total_i += $corte;
			$num_k++;
		}
...
function llamarGrafico(){
	if(parseInt(document.f1.anio_i.value)>parseInt(document.f1.anio_f.value)){
		...
		location.href="est_a_o_fran.php?&tipoGrafico="+document.f1.tipoGrafico.value+"&inicio_a="+document.f1.anio_i.value+"&fin_a="+document.f1.anio_f.value+"&id_det_orden="+document.f1.id_det_orden.value+"&id_tipo_orden="+document.f1.id_tipo_orden.value+"&";
	}
}
...
function imprimirGrafico(){
	...
	if(parseInt(document.f1.anio_i.value)>parseInt(document.f1.anio_f.value)){
		...
		location.href="est_a_o_fran_impreso.php?&tipoGrafico="+document.f1.tipoGrafico.value+"&inicio_a="+document.f1.anio_i.value+"&fin_a="+document.f1.anio_f.value+"&id_det_orden="+document.f1.id_det_orden.value+"&id_tipo_orden="+document.f1.id_tipo_orden.value+"&";
	}
}
</script>
...
<table border="0" width="760px" align="center"> 
	<tr>
		<td colspan="2" ALIGN="CENTER">
			<div class="cabe">ESTADISTICAS ANUALES DE <?php echo strtoupper(utf8_decode($nombre_det_orden));?> POR FRANQUICIAS</div>
		</td>
	</tr>
	<tr>
