...
$x->setQuery("id_cont_serv,id_serv,id_contrato,tipo_servicio,nombre_servicio,tipo_paq,cant_serv,costo_cobro,(cant_serv * costo_cobro) as total", "vista_cont_serv","id_cont_serv","id_contrato='$id_contrato' and status_con_ser='CONTRATO'");
...
//$x->addStandardControl(EyeDataGrid::STDCTRL_DELETE, "eliminarcontrato_servicio('%id_cont_serv%','%id_serv%','%id_contrato%')");
$x->addRowSelect("eliminarcontrato_servicio('%id_cont_serv%','%id_serv%','%id_contrato%')");
$x->hideFooter(true);
$x->printTable();

	
if($modo!='EXCEL'){
	$acceso->objeto->ejecutarSql("select count(*) as cont_serv from contrato_servicio, servicios where contrato_servicio.id_serv = servicios.id_serv and id_contrato='$id_contrato' AND  status_serv='ACTIVO'  and tipo_costo='COSTO MENSUAL' AND tipo_paq='PAQUETE BASICO'");
	if($row=row($acceso)){
