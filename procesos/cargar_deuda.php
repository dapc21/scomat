...
$id_cd= $ini_u.verCodLong($acceso,"id_cd");
...
$x->setQuery("id_cd,nombre_servicio,cantidad,costo,(cantidad*costo) as total", "cargar_deuda, servicios","id_cd","cargar_deuda.id_serv= servicios.id_serv and id_contrato='$id_contrato'");
$x->hideColumn('id_cd');
...
$x->addStandardControl(EyeDataGrid::STDCTRL_DELETE, "eliminarcargar_deuda('%id_cd%')");
$x->hideFooter(true);
$x->printTable();
...
$acceso->objeto->ejecutarSql("select sum(cant_serv * costo_cobro) as total from cargar_deuda where id_contrato='$id_contrato' and status_con_ser='CONTRATO'");
	$total=0;
	if($row=row($acceso)){
