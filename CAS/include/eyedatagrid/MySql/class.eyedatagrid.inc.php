...
class EyeDataGrid
{
	...
	*/
	private function buildHeader()
	{
		...
		{
			$this->column_count++;
			echo '<td class="tbl-header"><a href="javascript:;" ><strong>&nbsp;#&nbsp;</strong></a></td>';
		}
		...
		{
			$this->column_count++;
			echo '<td class="tbl-header-check" ><input type="checkbox" name="checkall" onclick="tblToggleCheckAll()"></td>';
		}
	...
	*/
	private function buildFooter($shown, $first = 0, $last = 0)
	{
		...
		echo '<tfoot><tr class="tbl-footer"><td class="tbl-nav" colspan="' . $this->column_count . '"><table width="100%" class="tbl-footer"><tr><td width="33%" class="tbl-found">Encontrados <em>' . $this->row_count . '</em> ';

		if ($this->row_count > 0)
			echo ', [<em>' . $first . '</em> - <em>' . $last . '</em>]';
	...
	public function printTable()
	{
		...
		if ($this->row_count == 0)
			...
			while ($row = $this->_db->fetchAssoc($this->result))
			{
				...
				if ($this->show_checkboxes)
					echo '<td width="35px" align="center"><input type="checkbox" class="tbl-checkbox" name="checkbox" value="' . $row[$this->primary] . '"></td>';
