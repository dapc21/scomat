...
function verbroadcaster($acceso){
	$cad='';
	$acceso->objeto->ejecutarSql("select *from broadcaster order By broadcasterid");
	while ($row=row($acceso)){
		$cad=$cad.opcion(trim($row["broadcasterid"]),trim($row["broadcasterid"])." : ".trim($row["broadcasterds"]));
	}
	return $cad;	
}

function verchannel($acceso){
	$cad='';
	$acceso->objeto->ejecutarSql("select *from channel order By channelid");
	while ($row=row($acceso)){
		$cad=$cad.opcion(trim($row["channelid"]),trim($row["channelid"])." : ".trim($row["channelds"]));
	}
	return $cad;	
}
function versubscription($acceso){
	$cad='';
	$acceso->objeto->ejecutarSql("select *from subscription order By subscriptionid");
	while ($row=row($acceso)){
		$cad=$cad.opcion(trim($row["subscriptionid"]),trim($row["subscriptionid"])." : ".trim($row["subscriptionds"]));
	}
	return $cad;	
}

function verproduct($acceso){
	$cad='';
	$acceso->objeto->ejecutarSql("select *from product order By productid");
	while ($row=row($acceso)){
		$cad=$cad.opcion(trim($row["productid"]),trim($row["productid"])." : ".trim($row["productds"]));
	}
	return $cad;	
}

function verevent($acceso){
	$cad='';
	$acceso->objeto->ejecutarSql("select *from event order By eventid");
	while ($row=row($acceso)){
		$cad=$cad.opcion(trim($row["eventid"]),trim($row["eventid"])." : ".trim($row["eventds"]));
	}
	return $cad;	
}
