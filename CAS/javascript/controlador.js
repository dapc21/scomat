...
function verificar_cas(tipoDato,clase)
{
  switch(clase)
  {
	...
	case "increaseIPPVbalance":
		...
		break;
	case "Product":
		if(validaCampo(document.f1.productId,isAlphanumeric) && validaCampo(document.f1.productDs,isTexto) && valdate(validityDateBegin()) && valdate(validityDateEnd()) && valdate(purchaseDateBegin()) && valdate(purchaseDateEnd()) && validaCampo(document.f1.genreId,isInteger) && validaCampo(document.f1.subgenreId,isInteger) && validaCampo(document.f1.price,isNumber) && validaCampo(document.f1.maxEvents,isInteger) && validaCampo(document.f1.dato,isTexto))
		{
		...
		break;
	case "Purchase":
		if(validaCampo(document.f1.idPurchase,isAlphanumeric) && validaCampo(document.f1.SMCid,isAlphanumeric) && valdate(statusDate()) && validaCampo(document.f1.dato,isTexto))
		{
			if(confirmacion_cas(tipoDato,clase,idPurchase()+"=@"+verRadiotipoCanal()+"=@"+ver_tipo_purchase()+SMCid()+"=@"+statusId()+"=@"+formatdate(statusDate())+"=@"+data()))
				document.f1.reset();
		}
		break;
	case "Event":
		if(validaCampo(document.f1.eventId,isAlphanumeric) && validaCampo(document.f1.title,isTexto) && valdate(broadcastBegin()) && valdate(broadcastEnd())  && validaCampo(document.f1.genreId,isInteger) && validaCampo(document.f1.subgenreId,isInteger) && validaCampo(document.f1.previewDuration,isInteger) && validaCampo(document.f1.dato,isTexto))
		{
		...
		break;
	case "Subscription":
		if(validaCampo(document.f1.subscriptionId,isAlphanumeric) && validaCampo(document.f1.subscriptionDs,isTexto)  && valdate(purchaseDateBegin()) && valdate(purchaseDateEnd()) && validaCampo(document.f1.price,isNumber)  && validaCampo(document.f1.dato,isTexto))
		{
		...
		break;
	case "CASSTBBean":
		if(validaCampo(document.f1.stbTypeId,isAlphanumeric) && validaCampo(document.f1.stbManufacturerId,isInteger) && validaCampo(document.f1.serialNumber,isTexto) && validaCampo(document.f1.barcode,isTexto) && validaCampo(document.f1.stbMasterTypeId,isInteger) && validaCampo(document.f1.stbMasterManufacturerId,isInteger) && validaCampo(document.f1.serialNumberMaster,isTexto) && validaCampo(document.f1.dato,isTexto))
		{
	...
	case "CASTimeRangeBean":
		if(validaCampo(document.f1.idCASTimeRangeBean,isAlphanumeric) && validaCampo(document.f1.subscriptionId,isAlphanumeric) && validaCampo(document.f1.broadcastTimeBegin,isTexto) && validaCampo(document.f1.broadcastTimeEnd,isTexto) && validaCampo(document.f1.dato,isTexto))
		{
	...
	case "SubscriptionChannel":
			if(confirmacion_cas(tipoDato,clase,subscriptionId()+"=@"+channelId()+"=@"+data()))
				document.f1.reset();
...
function habilita_tipo_purchase(){
	if (verRadiotipoCanal()=='SUBSCRIPTION'){
			document.f1.subscriptionId.disabled=false;
			document.f1.productId.disabled=true;
	}
	else{
			document.f1.subscriptionId.disabled=true;
			document.f1.productId.disabled=false;
	}
}
...
function listarCanalSusc(){
			params = ''; tblpage = ''; tblorder = ''; tblfilter = '';
			archivoDataGrid="procesos/datagrid_SubscriptionChannel.php?subscriptionid="+subscriptionId()+"&";
			updateTable_cas();
}
