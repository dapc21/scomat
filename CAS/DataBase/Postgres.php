      ...
      class Postgres extends DataAccess implements DataBaseMetodos
      {
      	...
      	public function registro_auditoria($sql)
      	{
			...
			if($tipo_sql=='INSERT' || $tipo_sql=='DELETE' || $tipo_sql=='UPDATE'){
				...
				if($tabla!=strtoupper("SINCRONIZACION_SERVI")){
					$res=@pg_query($this->conexion_alt, "insert into auditoria_comp(login,dir_ip,nom_equipo,fecha,hora,tipo_sql,tabla,refer,sql_comp) values ('$login','$dir_ip','$nom_equipo','$fecha','$hora','$tipo_sql','$tabla','$refer','$sql_comp')");
				}
