...
if($id_franq!='' || $id_zona!='' || $id_sector!='' || $id_calle!='' || $edificio!=''){
	//,nombre_franq 
...
and (id_franq = '1%') and (id_zona ILIKE '%0%') and (id_sector ILIKE '%0%') and (id_calle ILIKE '%0%');
	*/
	$sql=" 
	SELECT nro_contrato,id_franq,apellido,nombre,status_contrato,
	...
	,nombre_calle,nombre_sector,nombre_zona,edificio,etiqueta,telefono,direc_adicional,numero_casa,numero_piso
  ...
  ";
	...
	if($id_franq!=''){
		...
		$where=$where. "(edificio ILIKE '%$edificio%')";
		$tipo='edificio';
	}
	...
	if($tipo=='id_franq'){
		...
		if($edificio!=''){
			$where=$where. " and (edificio ILIKE '%$edificio%')";
		}
