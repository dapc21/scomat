...
class PDF extends JavaPrint
{
	...
	function Header()
	{
		...
		$this->MultiCell(260,5,nombre_empresa(),'0','C');
		$this->SetFont('Arial','',10);
		$this->MultiCell(260,5,tipo_serv(),'0','C');
	}
	...
	function Titulo()
	{
		...
		$this->MultiCell(260,7,strtoupper(_('cierre de puntos de cobros')),'0','C');
	...
	function TituloCampos()
	{
		...
		$w=array(10,18,17,20,47,30,60,15,20,17,18);
		$header=array(strtoupper(_('nro'),strtoupper(_('nro cont.')),strtoupper(_('nro fact.')),strtoupper(_('cedula')),strtoupper(_('nombre y apellido')),strtoupper(_('sector')),strtoupper(_('descripcion')),strtoupper(_('monto')),strtoupper(_('hora')),strtoupper(_('status')));
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$top=55;
			...
			$this->SetX($right)	;
				...
				$this->SetX($right);
				$this->SetFont('Arial','B',9);
				$this->Cell(52,6,strtoupper(_("servicios")),"0",0,"L");
		...
		while ($row=row($acceso))
		{
			...
			*/
			...
			//	$y++;
			//	echo trim($row1["nombre_servicio"]).":";
				$id_calle=trim($row["id_calle"]);
				$cable->objeto->ejecutarSql("select nombre_sector from vista_calle where id_calle='$id_calle'");
				if($row1=row($cable))
					$nombre_sector=trim($row1['nombre_sector']);
					
				$cable->objeto->ejecutarSql("SELECT nombre_servicio,id_serv,fecha_inst FROM vista_pago_ser where id_pago='$id_pago'");
				$nombre_servicio='';
				...
				$this->Cell($w[5],6,$nombre_sector,"LR",0,"J",$fill);
				$this->Cell($w[6],6,$nombre_servicio,"LR",0,"J",$fill);
				$this->Cell($w[7],6,number_format(trim($row["monto_pago"])+0, 2, ',', '.'),"LR",0,"J",$fill);
				//$this->Cell($w[8],6,formatofecha(trim($row["fecha_pago"])),"LR",0,"J",$fill);
				$this->Cell($w[8],6,utf8_decode(trim($row["hora_pago"])),"LR",0,"J",$fill);
				$this->Cell($w[9],6,utf8_decode(trim($row["status_pago"])),"LR",0,"J",$fill);
