...
class PDF extends JavaPrint
{
	...
	function Cuerpo($acceso,$id_contrato_serv,$nro_fact)
	{
		$this->fuente='courier';
		$this->tipo='may';
...
*/
		...
		$cont=lectura($acceso,"SELECT nombre_zona,nombre_sector,nombre_calle,numero_casa,telefono FROM vista_contrato where id_contrato='$id_contrato' LIMIT 1 offset 0");
...
//		echo "<br><br>SELECT tipo_pago.tipo_pago, detalle_tipopago.banco,detalle_tipopago.numero FROM tipo_pago,detalle_tipopago where id_pago='$id_pago' and detalle_tipopago.id_tipo_pago = tipo_pago.id_tipo_pago";
		...
		$this->add(strtoupper(_("datos de cliente")),65,$alto);				
		$this->add(strtoupper(_("datos de cliente")),360,$alto);
		$alto+=$inter;
		...
		$alto+=$inter+5;
		$this->add(strtoupper(_("factura control")).": $nro_factura",$x1,$alto);
		$this->add(strtoupper(_("factura control")).": $nro_factura",$y1,$alto);
		$alto+=$inter;
...
$pdf->Cuerpo($acceso,$id_contrato_serv,$nro_fact);
