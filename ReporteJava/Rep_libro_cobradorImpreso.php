...
class PDF extends JavaPrint
{
	...
	function Cuerpo($acceso,$desde,$hasta,$id_persona_cob)
	{
		...
		$this->SetFont('Arial','B',10);
		$this->SetX(20);
		$this->Cell(40,5,strtoupper(_("cobrador")).": ".$nombre."     ".strtoupper(_("c.i.")).": ".$ncedula,0,0,'L');
		$this->Ln();
		$this->SetFont('Arial','B',10);
		$this->SetX(20);
		...
		$this->Ln();
		$this->SetX(75);
		$this->Cell(30,6,strtoupper(_('total iva')).' (12%):',"1",0,"L",$fill);
		
		$this->Cell(25,6,number_format($iva+0, 2, ',', '.'),"1",0,"R",$fill);
		
		$this->Ln();
		$this->SetX(75);
		$this->Cell(30,6,strtoupper(_('comision ')).'(5%):',"1",0,"L",$fill);
		$this->Cell(25,6,number_format($iva+0, 2, ',', '.'),"1",0,"R",$fill);
