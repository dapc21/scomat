...
class PDF extends JavaPrint
{
	...
	function Header()
	{
		...
		$this->MultiCell(190,7,utf8_decode(_('libro de ventas')),'0','C');
	...
	function TituloCampos()
	{
		...
		$header=array(_('nro'),_('factura'),_('cedula'),_('nombre'),_('base'),_('iva'),_('total'),_('fecha'),_('status'));
	...
	function Cuerpo($acceso,$desde,$hasta)
	{
		...
		$acceso->objeto->ejecutarSql("SELECT inicial_doc,fecha_pago,nro_factura,cedulacli,nombrecli,apellidocli,monto_pago,status_pago,tipo_cliente FROM vista_pago_cont where fecha_pago between '$desde' and '$hasta' order by nro_factura");
		...
		$this->Cell(55,6,_('resumen general'),"0",0,"C",$fill);
		
		$this->Ln();
		$this->SetX(75);
		$this->SetFont('Arial','B',9);
		$this->Cell(30,6,_('total base').':',"1",0,"L",$fill);
		
		$this->Cell(25,6,number_format($base+0, 2, ',', '.'),"1",0,"R",$fill);
		
		$this->Ln();
		$this->SetX(75);
		$this->Cell(30,6,_('total iva').':',"1",0,"L",$fill);
		
		$this->Cell(25,6,number_format($iva+0, 2, ',', '.'),"1",0,"R",$fill);
		
		$this->Ln();
		$this->SetX(75);
		$this->Cell(30,6,_('total general').':',"1",0,"L",$fill);
