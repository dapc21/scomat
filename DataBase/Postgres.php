...
*/
                        if($this->resultado=@pg_query($this->conexion, $sql))
                        {
                              ...
                              $error_html="<br>ERROR DE BASE DE DATOS: <br>: $sql <br>".pg_errormessage($this->conexion)."";
                              //echo $error_html;
                              $this->sql_error=$this->sql_error.$error_html;
                              $id = fopen("error_pg_saeco.txt","a+");
                              fwrite($id,"\n\n $error \n");
                              fclose ($id);
                              return false;
            ...
            public function ejecutarSqlObject($sql)
      	{
			$sql=strtoupper($sql);
			if ($this->conectado)
      		{
				if($this->resultado=@pg_query($this->conexion, $sql))
      			{
      				if($num=pg_affected_rows($this->resultado)>=0)
      				{						
      				  $this->rowset=pg_fetch_object($this->resultado);
      				  $this->registros=pg_num_rows($this->resultado);		 
      				  return true; 
      				}
      			}
			...
			//echo ":$sql:";
      		if ($this->conectado)
      		{
      			if($this->resultado=@pg_query($this->conexion, $sql))
      			{
					...
					$error ="\n(ERROR DE BASE DE DATOS: \n $sql <br>\n".pg_errormessage($this->conexion).")\n";
					//echo $error;
					$id = fopen("error_pg_saeco_sinc.txt","a+");
					fwrite($id,"\n\n$error \n");
					fclose ($id);
					return false;
				}
            ...
            function siguienteRegistro()
      {
      	...
      	function siguienteRegistroObject()
     	{
      		if($this->conectado)
     			{
      			if ($this->resultado)
