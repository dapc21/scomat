...
for($y=0;$y<$x;$y++)
{
	...
	if(class_exists($nombreClase))
	{
		...
		if(!$objeto->validaExistencia($acceso)){
			...
			if($boton=="incluir" || $nombreClase=="ModuloPerfil" || $nombreClase=="mov_mat"  || $nombreClase=="mov_mat_orden" ){
				//haces la llamada al metodo
				$objeto->$metodo($acceso);
				$mensaje=true;
			}
			else{
				echo ', EL CAMPO NO ESTA REGISTRADO EN ESTA TABLA:'.$nombreClase;
				$mensaje=false;
			}
		}
		else{
			if($boton!="incluir" || $nombreClase=="ModuloPerfil" || $nombreClase=="mov_mat_orden" ){
				$objeto->$metodo($acceso);
				$mensaje=true;
			}
			else{
				echo ', EL CAMPO YA ESTA REGISTRADO EN ESTA TABLA:'.$nombreClase;
				$mensaje=false;
			}
		}
	}
