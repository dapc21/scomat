...
class EyeDataGrid
{
	...
	*/
	public function __construct(EyePostgresAdap $_db, $image_path = '')
	{
		...
		if (empty($image_path))
			$this->image_path = 'saecomat/include/eyedatagrid/images/';
		else
	...
	*/
	private function buildHeader()
	{
		...
		if ($this->show_checkboxes)
		{
			$this->column_count++;
			if($this->clase=="Pagos"){
				echo '<th class="tbl-header-check numeric" align="center" ><input type="checkbox" name="checkbox" onclick="tblToggleCheckAll_mat();calcularMontoPago();"> <input  type="hidden" name="email" maxlength="40" size="10" value="" ></th>';
			}
				...
				echo '<th class="tbl-header-check" align="center" ><input type="checkbox" name="checkbox" onclick="tblToggleCheckAll_mat()"></th>';
			}
		}
		...
		foreach ($headers as $t)
		{
			...
			if ($this->hide_order){
				...
				if ($this->order and $this->order['Column'] == $t)
					...
					echo '<th><a href="javascript:;" onclick="tblSetOrder_mat(\'' . $t . '\', \'' . $order . '\',\''.$this->archivoDataGrid.'\',\''.$this->divDataGrid.'\')">' . strtoupper($header) . "</a>";
	...
	private function buildFooter($shown, $first = 0, $last = 0)
	{
		...
		if ($this->row_count > $this->results_per_page)
		{
			...
			if ($this->page > 1){
				echo '
				<li class="prev">
				<a href="javascript:;" onclick="tblSetPage_mat(1,\''.$this->archivoDataGrid.'\',\''.$this->divDataGrid.'\')"><i class="fa fa-angle-double-left"></i></a>
				</li>
				<li class="prev">
				<a href="javascript:;" onclick="tblSetPage_mat(' . ($this->page - 1) . ',\''.$this->archivoDataGrid.'\',\''.$this->divDataGrid.'\')"><i class="fa fa-angle-left"></i></a>
				</li>';
			}
			else{
				echo '
			...
			for ($i = $startpage; $i <= $endpage; $i++)
			{
				if ($i == $this->page){						
					...
					echo '
					<li>
					<a href="javascript:;" onclick="tblSetPage_mat(' . $i . ',\''.$this->archivoDataGrid.'\',\''.$this->divDataGrid.'\')">' . $i . '</a>
					</li>';
				}
			}
			...
			if($this->page < $pages){
				echo '
				<li class="next">
				<a href="javascript:;" onclick="tblSetPage_mat(' . ($this->page + 1) . ',\''.$this->archivoDataGrid.'\',\''.$this->divDataGrid.'\')"><i class="fa fa-angle-right"></i></a>
				</li>
				<li class="next">
				<a href="javascript:;" onclick="tblSetPage_mat(' . $pages . ',\''.$this->archivoDataGrid.'\',\''.$this->divDataGrid.'\')"><i class="fa fa-angle-double-right"></i></a>
				</li>';
			}
			else{
				echo '
		...
		echo '<button type="button" class="btn btn-success contenido-boton-excel" data-toggle="tooltip" data-placement="bottom" title="Exportar Datos a Excel" onclick=\'updateTableExcel("'.$this->archivoDataGrid.'")\'><i class="fa fa-file-text"></i> </button>' . ' ';	
		echo '<button type="button" class="btn btn-info contenido-boton-todos" data-toggle="tooltip" data-placement="bottom" title="Mostrar Todos los Registros" onclick="tblSetResulTodo_mat(\''.$this->archivoDataGrid.'\',\''.$this->divDataGrid.'\')"><i class="glyphicon glyphicon-list"></i> </button>';
		echo '
		</div>
		</div>
	...
	*/
	public function printTable()
	{
		...
		if (!$this->result)
		{
			/*echo*/ echo '<div style="color: red; font-weight: bold; border: 2px solid red; padding: 10px;">Oops! We ran into a problem while trying to output the table. <a href="javascript:;" onclick="tblReset_mat()">Click here</a> to reset the table or <a href="javascript:;" onclick="alert(\'' . @ereg_replace('[\'"]', '', $this->_db->error()) . '\')">here</a> to review the error.</div>';
			return;
		}
		...
		if ($this->reset_button)
			/*echo*/ echo '<span class="tbl-reset"><a href="javascript:;" onclick="tblReset_mat()" title="' . $this->reset_button .'"><img src="' . $this->image_path . $this->img_reset . '" class="tbl-reset-image">' . $this->reset_button .'</a></span>';
		...
		if ($this->row_count == 0)
			...
			while ($row = $this->_db->fetchAssoc($this->result))
			{
				...
				if ($this->show_row_number)
					echo '<td class="numeric" align="left">' . $line . '</td>';
				...
				if ($this->show_checkboxes){
					if($this->clase=="Pagos"){
						...
						/*echo*/ echo '<td class="numeric" align="center"><input type="checkbox" class="tbl-checkbox" name="checkbox" onchange="calcularMontoPago()" value="'.$row[$this->primary].'=@'.$total_pago.'"></td>';
					}
					else if($this->clase=="pedido"){
						/*echo*/ echo '<td class="numeric" align="center"><input type="checkbox" class="tbl-checkbox" name="checkbox" onchange="llamadoUno(\''.$row[$this->primary].'\',\''.$xxindice.'\')" value="'.$row[$this->primary].'"> </td>';
							$xxindice++;
					}else if($this->clase=="movimiento"){
						/*echo*/ echo '<td class="numeric" align="center"><input type="checkbox" class="tbl-checkbox" name="checkbox" onchange="llamadoUnoMovimiento(\''.$row[$this->primary].'\',\''.$xxindice.'\')" value="'.$row[$this->primary].'"> </td>';
							$xxindice++;
					}else if($this->clase=="mat_prov"){
						/*echo*/ echo '<td class="numeric" align="center"><input type="checkbox" class="tbl-checkbox" name="checkbox"  value="'.$row[$this->primary].'"> </td>';
							$xxindice++;
					}else if($this->clase=="confir_pedido"){
						/*echo*/ echo '<td class="numeric" align="center"><input type="checkbox" class="tbl-checkbox" name="checkbox" onchange="llamadoUno(\''.$row[$this->primary].'\',\''.$xxindice.'\')" value="'.$row[$this->primary].'"> </td>';
							$xxindice++;
					}else if($this->clase=="realizar_compra"){
						/*echo*/ echo '<td class="numeric" align="center"><input disabled type="checkbox" class="tbl-checkbox" name="checkbox" onchange="llamadoUno(\''.$row[$this->primary].'\',\''.$xxindice.'\')" value="'.$row[$this->primary].'"> </td>';
							$xxindice++;
					}else if($this->clase=="inventario"){
						/*echo*/ echo '<td class="numeric" align="center"><input disabled type="checkbox" Checked disabled class="tbl-checkbox" name="checkbox" onchange="" value="'.$row[$this->primary].'"> </td>';
							$xxindice++;
					}else if($this->clase=="aprobar_inven"){
						/*echo*/ echo '<td class="numeric" align="center"><input disabled type="checkbox" Checked disabled class="tbl-checkbox" name="checkbox" onchange="" value="'.$row[$this->primary].'"> </td>';
							$xxindice++;
					}
					else{
				...
				foreach ($row as $key => $value)
				{
					...
					if($this->clase=="Pagos" || $this->clase=="Actualizar_Pagos"  || $this->clase=="historialpago"  || $this->clase=="anular_pagos"){
						...
						if(trim($key)=="nombre"){
							...
							if(trim($key)=="c_uni_sal"){
								$c_uni_sal=trim($value);
							}
							if(trim($key)=="c_uni_ent"){
								$c_uni_ent=trim($value);
							}
							if(trim($key)=="us_abre"){
								$us_abre=trim($value);
							}
							if(trim($key)=="abreviatura"){
								$abreviatura=trim($value);
							}
							if(trim($key)=="id_m"){
								$id_m=trim($value);
							}
							if(trim($key)=="min"){
								$stock_min=trim($value);
								$stock_min2=$stock_min*$c_uni_sal;
								$value = '<input disabled type="hidden" name="id_m_'.$id_mat.'" id="id_m_'.$id_mat.'" value="'.$id_m.'"  maxlength="10" size="5">
								<input disabled type="hidden" name="min_'.$id_mat.'" id="min_'.$id_mat.'" value="'.$stock_min2.'"  maxlength="10" size="5">
								<input disabled style="text-align:center;color:cian;text-shadow: #C3C3C3 10px 10px 10px ;" border="0" type="text" onKeyPress="return validar_numeros(event)" name="minver_'.$id_mat.'" id="minver_'.$id_mat.'" value="'.$stock_min.'"  maxlength="10" size="5">'.strtolower($abreviatura).'';
							}
							if(trim($key)=="stock_min"){
								$value = '<div  style="width: 100%;float:center;"><div  style="width: 38%;float:left;">
								<input disabled type="hidden" name="cantNew_'.$id_mat.'" id="cantNew_'.$id_mat.'" value="0"  maxlength="10" size="5">
								<input disabled type="text" onKeyPress="return validar_numeros(event);" onKeyUp="calcuSalida(\''.$id_mat.'\',\''.$abreviatura.'\',\''.$us_abre.'\');"  onchange="validar_can_movimien(\''.$id_mat.'\',\''.$cantindice.'\',\''.$c_uni_sal.'\',\''.$abreviatura.'\',\''.$us_abre.'\');" name="cant_'.$id_mat.'" id="cant_'.$id_mat.'"  maxlength="5" size="5" value="">'.strtolower($abreviatura).'</div>';
								if($abreviatura!=$us_abre){
								$value = $value .'<div  style="width:  58%;float:right;">
								<input disabled type="text" onKeyPress="return validar_numeros(event);" onKeyUp="calcuSalida02(\''.$id_mat.'\',\''.$abreviatura.'\',\''.$us_abre.'\');"  onchange="validar_can_movimien(\''.$id_mat.'\',\''.$cantindice.'\',\''.$c_uni_sal.'\',\''.$abreviatura.'\',\''.$us_abre.'\');" name="cantabre_'.$id_mat.'" id="cantabre_'.$id_mat.'" value=""  maxlength="20" size="4">'.strtolower($us_abre).'</div>';
								}
								$cantindice++;
							}
							if(trim($key)=="stock"){
								...
								$value = '<div  style="width: 100%;float:center;"><div  style="width: 38%;float:left;">								
								<input disabled type="hidden" name="cent'.$id_mat.'" id="cent'.$id_mat.'" value="'.$c_uni_ent.'"  maxlength="10" size="5">
								<input disabled type="hidden" name="stock_'.$id_mat.'" id="stock_'.$id_mat.'" value="'.$stock.'"  maxlength="10" size="5">
								<input disabled type="hidden" name="csal'.$id_mat.'" id="csal'.$id_mat.'" value="'.$c_uni_sal.'"  maxlength="10" size="5">
								<input disabled style="text-align:center;color:cian;text-shadow: #C3C3C3 10px 10px 10px ;" border="0" type="text" title="'.$c_uni_ent.' '.$abreviatura.' = '.$c_uni_sal.' '.$us_abre.'" onKeyPress="return validar_numeros(event)" name="stockver_'.$id_mat.'" id="stock_'.$id_mat.'" value="'.$stock1[0].'"  maxlength="20" size="6">'.strtolower($abreviatura).'</div>';
								if($abreviatura!=$us_abre){
								$value = $value .'<div  style="width:  58%;float:right;">
								<input disabled style="text-align:center;color:cian;text-shadow: #C3C3C3 10px 10px 10px ;" border="0" type="text" onKeyPress="return validar_numeros(event)" name="stock2_'.$id_mat.'" id="stock2_'.$id_mat.'" value="'.$stock2.'"  maxlength="20" size="6">'.strtolower($us_abre).'</div>';
								}
							}
						}else if($this->clase=="realizar_compra"){
							if(trim($key)=="c_uni_sal"){
							...
							if(trim($key)=="cant_ped"){
								$cant_ped=trim($value);
								$value = '<input disabled type="text"  onKeyPress="return validar_numeros(event)"  name="cant2_'.$id_mat.'" id="cant2_'.$id_mat.'"  maxlength="5" size="3" value='.$cant_ped.'>';
							}
							if(trim($key)=="stock_min"){
								$value = '<input disabled type="hidden" name="csal'.$id_mat.'" id="csal'.$id_mat.'" value="'.$c_uni_sal.'"  maxlength="10" size="5">
								<input disabled type="hidden" name="cantNew_'.$id_mat.'" id="cantNew_'.$id_mat.'" value="'.$cant_ped*$c_uni_sal.'"  maxlength="10" size="5">
								<input  type="text" onchange="calcuSalida_Pedido(\''.$id_mat.'\');" onKeyPress="return validar_numeros(event)" onkeyup="calculaTotalCP();calcuSalida_Pedido(\''.$id_mat.'\');" name="cant_'.$id_mat.'" id="cant_'.$id_mat.'"  maxlength="5" size="3" value="">';
							}
							if(trim($key)=="precio"){
								$precio=trim($value);
								$value = '<input  type="text" onKeyPress="return validar_numeros(event);" onkeyup="calculaTotalCP();" name="precio_'.$id_mat.'" id="precio_'.$id_mat.'"  maxlength="10" size="10" value="">';
							}
							if(trim($key)=="abreviatura"){
								$precio=trim($value);
								$value = '<input  type="text"  onKeyPress="return validar_numeros(event);" onkeyup="calculaTotalCP2();" name="preciot_'.$id_mat.'" id="preciot_'.$id_mat.'"  maxlength="10" size="10" value="">';
							}
							...
							if(trim($key)=="precio"){
								$precio=trim($value);
								$value = '<input  type="text" onKeyPress="return validar_numeros(event);" onkeyup="" name="canreal_'.$id_mat.'" id="canreal_'.$id_mat.'"  maxlength="6" size="10" value="'.$precio.'">';
							}
							if(trim($key)=="abreviatura"){
								$abreviatura=trim($value);
								$valueJ = $valueJ.'<div id="just_'.$id_mat.'" style="display:none;"><textarea    name="justifi_'.$id_mat.'" id="justifi_'.$id_mat.'"  maxlength="150" size="30" value=""></textarea></div>';
							}
							if(trim($key)=="stock"){
								...
								$stock1=explode('.',$stock/$c_uni_sal);
								$stock2=$stock%$c_uni_sal;
								$value = $valueJ.'<div  style="width: auto%;float:center;"><div  style="width: 39%;float:left;">								
								<input disabled type="hidden" name="cent'.$id_mat.'" id="cent'.$id_mat.'" value="'.$c_uni_ent.'"  maxlength="10" size="5">
								<input disabled type="hidden" name="stock_'.$id_mat.'" id="stock_'.$id_mat.'" value="'.$stock.'"  maxlength="10" size="5">
								<input disabled type="hidden" name="csal'.$id_mat.'" id="csal'.$id_mat.'" value="'.$c_uni_sal.'"  maxlength="10" size="5">
								<input disabled style="text-align:center;color:cian;text-shadow: #C3C3C3 10px 10px 10px ;" border="0" type="text" title="'.$c_uni_ent.' '.$abreviatura.' = '.$c_uni_sal.' '.$us_abre.'" onKeyPress="return validar_numeros(event)" name="stockver_'.$id_mat.'" id="stock_'.$id_mat.'" value="'.$stock1[0].'"  maxlength="20" size="6">'.$abreviatura.'</div>';
								if($abreviatura!=$us_abre){
								$value = $value .'<div  style="width:  60%;float:left;">
								<input disabled style="text-align:center;color:cian;text-shadow: #C3C3C3 10px 10px 10px ;" border="0" type="text" onKeyPress="return validar_numeros(event)" name="stock2_'.$id_mat.'" id="stock2_'.$id_mat.'" value="'.$stock2.'"  maxlength="20" size="6">'.$us_abre.'</div>';
								}
							}				  
						}else if($this->clase=="aprobar_inven"){
							if(trim($key)=="id_mat"){
							...
							if(trim($key)=="stock_min"){
								...
								$stock1=explode('.',$stock/$c_uni_sal);
								$stock2=$stock%$c_uni_sal;
								$value = '<div  style="width: auto;float:center;"><div  style="width: 48%;float:left;">
								<input disabled type="hidden" name="canreal_'.$id_mat.'" id="canreal_'.$id_mat.'" value="'.$stock.'"  maxlength="10" size="5">
								<input disabled type="hidden" name="cantNew_'.$id_mat.'" id="cantNew_'.$id_mat.'" value="0"  maxlength="10" size="5">
								<input  type="text" onKeyPress="return validar_numeros(event);" onKeyUp="calcuSalida(\''.$id_mat.'\',\''.$abreviatura.'\',\''.$us_abre.'\');"  onchange="" name="cant_'.$id_mat.'" id="cant_'.$id_mat.'"  maxlength="5" size="5" value="">'.$abreviatura.'</div>';
								if($abreviatura!=$us_abre){
								$value = $value .'<div  style="width:  48%;float:left;">
								<input  style="text-align:center;color:cian;text-shadow: #C3C3C3 10px 10px 10px ;" onKeyUp="calcuSalida02(\''.$id_mat.'\',\''.$abreviatura.'\',\''.$us_abre.'\');"  onchange="" border="0" type="text" onKeyPress="return validar_numeros(event)" name="cantabre_'.$id_mat.'" id="cantabre_'.$id_mat.'" value=""  maxlength="20" size="4">'.$us_abre.'</div>';
								}
								$cantindice++;
							}
							if(trim($key)=="min"){
								$value = '<input  type="button"  onclick="muestrajusti(\'just_'.$id_mat.'\',\'but_'.$id_mat.'\');" id="but_'.$id_mat.'" value=">>">';
								$value = $value.'<div id="just_'.$id_mat.'" style="display:none;"><textarea    name="justifi_'.$id_mat.'" id="justifi_'.$id_mat.'"  maxlength="150" size="30" value="">..</textarea></div>';
							}
							if(trim($key)=="stock"){
							...
							if(trim($key)=="stock"){
								$stock=trim($value);
								$valor=explode('.',$stock/$c_uni_sal);
								$value=$valor[0];
								
							}
							if(trim($key)=="stock_min"){
								$value = '<input disabled type="text" onKeyPress="return validar_numeros(event)" name="cant_'.$id_mat.'" id="cant_'.$id_mat.'"  maxlength="5" size="5" value="">';
							}
