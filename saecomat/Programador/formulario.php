...
function radio_mat($etiqueta,$nombre1,$nombre2,$name){	
...
function registrar($clase){	
	return boton("REGISTRAR","registrar",'verificar_mat(\'incluir\',\''.$clase.'\')',"button");
}
function modificar($clase){	
	return boton("MODIFICAR","modificar",'verificar_mat(\'modificar\',\''.$clase.'\')',"button");
}
function eliminar($clase){	
	return boton("ELIMINAR","eliminar",'verificar_mat(\'eliminar\',\''.$clase.'\')',"button");
}
function cancelar(){
...
function plantilla($tipo){
	...
	if($tipo!="fecha")
		$cadena = $cadena.fila(columna(etiqueta("Tipo de Dato")).columna(tipoDato())).fila(columna(etiqueta("Longitud:&nbsp;&nbsp;").input(tipo("text").nombre("longitud").maximo(50).tamano(5).valor("10"))).columna(etiqueta("Precisi&oacute;n:&nbsp;&nbsp;").campoSolo("precision",5).campoOculto('valornulo','null')));
