...
function Imprimir'.$nombreRep.'(){
		location.href="saecomat/reportes/'.$nombreRep.'Impreso.php";
}
');
	fclose ($id);
	fclose ($fp);
...
function crearClaseControladora($cadena)
{
	...
	{	
		...
		if(strstr($linea,'function validarDato(){')){
			fwrite($id,'function validar'.$cadena[1].'(){ conexionPHP_mat("validarExistencia.php","1=@'.strtolower($cadena[1]).'","dato=@"+data());}
');
			$x=true;
		}
		...
		if($x==true)
		{
			if(strstr($linea,'cabecera().'))
			{
				if($tipo=='campoClave'){
					fwrite($id,$linea);
					if($cadena[14]=='true'){
						fwrite($id,'		    campoClave("'.$cadena[4].'","'.$cadena[3].'",'.$cadena[11].','.$cadena[10].',"onChange","validar'.$cadena[1].'()",\'<?php $acceso->objeto->ejecutarSql("select *from '.strtolower($cadena[1]).' ORDER BY '.strtolower($cadena[3]).' desc LIMIT 1 offset 0"); echo "CO".verCo($acceso,"'.strtolower($cadena[3]).'")?>\').
');
					}
					else{
...
function agregarCampoControlador($cadena,$tipo)
{
	...
	while ($linea=fgets($fp))
	{
		...
		if(strstr($linea,'function validar'.$cadena[1].'(){')){
				if($tipo=='campoClave'){
					fwrite($id,'function validar'.$cadena[1].'(){ conexionPHP_mat("validarExistencia.php","1=@'.strtolower($cadena[1]).'","'.$cadena[3].'=@"+'.$cadena[3].'());}
');
					$y=false;
				}
		}
...
');
		...
		if($x==true)
		{
			if(strstr($linea,'confirmacion_mat(tipoDato,clase'))
			{
			  if($cadena[2]=='radio'){
...
');	
				...
				fwrite($id,'			asignarCheck_mat(cadena['.$valor++.']);
');	
				}
				else if($cadena[2]=='fecha'){
	...
	while ($linea=fgets($fp))
	{
		...
		if($x==true)
		{
			if(strstr($linea,'confirmacion_mat(tipoDato,clase'))
			{
				if($cadena[2]=='radio')
				{
					$linea = str_replace('+"=@"+verradio_mat()','',$linea);				
				}
				else
					$linea = str_replace('+"=@"+'.$cadena[3].'()','',$linea);				
...
function eliminarCampoValidacion($cadena)
{
	...
	while ($linea=fgets($fp))
	{
		...
		if($x==true)
		{
			...
			if($cadena[2]=='casilla')
			{
				if(strstr($linea,'asignarCheck_mat('))
				{
					$y=false;
					$z=true;
				}
