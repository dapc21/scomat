...
?>
<input  type="hidden" name="id_mov2" id="id_mov2" maxlength="10" size="30"onChange="validarmovimiento()" value="<?php $acceso->objeto->ejecutarSql("select *from movimiento_orden where (id_mov ILIKE '$ini_u%') ORDER BY id_mov desc LIMIT 1 offset 0"); echo $ini_u.verCodLong2($acceso,"id_mov")?>">
...
<fieldset >
			  <legend ><?php echo _("materiales utilizados en la realizacion de la orden m");?></legend>	
<table border="0" width="97%" align="CENTER" > 

				<table border="0" width="97%" align="CENTER" cellpadding="4" cellspacing="4"> 
				<input  type="hidden"  name="id_mov" id="id_mov" maxlength="10" size="25"onChange="validarmovimiento()" value="<?php $acceso->objeto->ejecutarSql("select *from movimiento_orden where (id_mov ILIKE '$ini_u%') ORDER BY id_mov desc LIMIT 1 offset 0"); echo $ini_u.verCodLong($acceso,"id_mov")?>">
				<input style="display:none;" type="checkbox" name="checkboxTrans" id="checkboxTrans" onclick="validaTransfer(this.id);">
				<input  type="hidden" name="id_tm" id="id_tm" maxlength="15" size="30" value="A0000002==ENTRADA" >
				...
				<input  type="hidden" name="tipo_mov" maxlength="20" size="30" value="MATERIALES" >
					...
					<input  type="hidden" name="iddep"  id="iddep" maxlength="15" size="30" value="A0000001" onchange="valIqualD();traermat();">
							...
							<table width="97%" cellspacing="1" cellpadding="1" border="0" align="CENTER">
								<tr>
									...
									<td style="width: 25%;">
										...
										<input type="hidden" size="25" maxlength="30"  disabled>
									
									
										<input id="minver" type="hidden" size="5" maxlength="10" value="0" name="minver" onkeypress="return validar_numeros(event)" disabled="">
							...
							<div align="center" style=" float:center; width: 100%;">
									<div  style="display:block;width: 48%;float:left;border:1px; ">
										<div id="idregis1" style="display:block;width: auto;float:right;">
											<input  type="button" name="registrar_m" id="registrar_m" value="<?php echo _("agregar");?>" onclick="verificar_mat('incluir','movimiento_orden')">
										</div>
										<div id="idregis2" style="display:none;width: auto;float:right;">
											<input  type="button" name="registrar1" id="registrar1" value="<?php echo _("agregar");?>" onclick="verificar_mat('incluir',movimiento_orden_transferencia')">&nbsp;
										</div>
									</div>
