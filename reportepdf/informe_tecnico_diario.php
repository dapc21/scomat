...
*/
	...
	function analisis_vencimiento_cortado($acceso,$fecha,$id_f,$setx,$sety)
	{
		$cable=conexion();
		$cable1=conexion();
		$w=array(46,20,30);	
		...
		$this->SetFont('Arial','BIU',8);
		$this->SetX($right);
		$this->Cell($right,6,strtoupper(_('DEUDA DE ABONADOS CORTADOS')),"0",0,"L");
...
$pdf->analisis_vencimiento_cortado($acceso,$fecha,$id_f,110,70);

$pdf->ordenes_asignada_impresa($acceso,$fecha,$id_f,10,58);
$pdf->ordenes_mes($acceso,$fecha,$id_f,10,115);
$pdf->ordenes_mes_anterior($acceso,$fecha,$id_f,110,115);

$pdf->cortes_generales($acceso,$fecha,$id_f,10,225);
$pdf->reconexiones_generales($acceso,$fecha,$id_f,146,225);

$pdf->proceso_corte($acceso,$fecha,$id_f,77,225);

$pdf->AddPage('P','letter');
$pdf->ordenes_pendientes_semanas($acceso,$fecha,$id_f,10,30);
