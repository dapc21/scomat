	...
	function Cuerpo($acceso,$where,$deuda,$desde,$hasta,$titulo_list,$salto_p,$cobrador)
	{
		...
		for($i=0;$i<count($dato);$i++){
			...
			//echo "$salto_p:";
			 ...
			 $det_deuda="";
			 $fecha_act=date("Y-m-01");
			 //echo "SELECT nombre_servicio,costo_cobro,tipo_costo,tipo_paq,fecha_inst FROM pago_servicio, contrato_servicio_deuda, servicios   WHERE  pago_servicio.id_cont_serv = contrato_servicio_deuda.id_cont_serv  AND contrato_servicio_deuda.id_serv = servicios.id_serv and id_contrato='$id_contrato'  and fecha_inst < '$fecha_act'<br>";
			 $cable->objeto->ejecutarSql("SELECT nombre_servicio,costo_cobro,tipo_costo,tipo_paq,fecha_inst FROM  contrato_servicio_deuda, servicios   WHERE   contrato_servicio_deuda.id_serv = servicios.id_serv and id_contrato='$id_contrato'  and fecha_inst < '$fecha_act' order by fecha_inst");
			...
			//	echo "SELECT sum(costo_cobro*cant_serv) as costo_cobro  FROM contrato_servicio_deuda where  id_contrato='$id_contrato' and status_con_ser='DEUDA' and fecha_inst between '$fec_ini' and '$fec_fin'" ;
				...
				if($costo_cobro>0){
					$det_deuda = $det_deuda."M $mes_l:  $costo_cobro;        ";
				}
		...
		//	ordenDeCorte($acceso,$id_contrato,$tecnico);
			...
			$total_deuda = $total_deuda+(trim($dato[$i]["deuda"])+0);
			...
			$this->Ln();	
			$this->SetFont('Arial','B',8);
			$this->MultiCell(193,5,"POSTE: $postel ; DETALLE DEUDA: $det_deuda      $fecha_ven","1","J",$fill);
		...
		*/
		...
		$total_deuda=number_format($total_deuda+0, 2, ',', '.');
		$this->Ln();
		$this->SetFont('Arial','B',9);
		$this->MultiCell(193,5,"TOTAL DEUDA:  $total_deuda","0","J");
