...
class PDF extends FPDF
{
	...
	function Titulo($titulo_cierre)
	{
		...
		$this->MultiCell(190,7,strtoupper($titulo_cierre),'0','C');
...
*/
		...
		while($row=row($cable)){
			...
			if($id_f!='0'){
				$titulo_cierre="informe de penetracion por zona / status DE $nombre_franq";
			}
			...
			$pdf->Output("../archivos/$id_f/$fecha informe_penetracion.pdf",'F');
		}
		
			$id_f='0';
			
				$titulo_cierre='informe de penetracion por zona / status GENERAL';
			...
			$pdf->Output("../archivos/general/$fecha informe_penetracion.pdf",'F');
?> 