...
class PDF extends FPDF
{
        ...
        function Cuerpo($acceso,$id_orden,$id_gt)
        {
                ...
                if($row=row($acceso)){
                        ...
                        $status_orden=utf8_decode(trim($row['status_orden']));

                        $this->Titulo($nombre_det_orden,$id_orden);
				...
				$fecha_imp=formatofecha($fecha_imp);
                $acceso->objeto->ejecutarSql("SELECT contrato_fisico,taps,pto,edificio,telefono,postel,telf_casa,id_persona,cod_id_persona FROM vista_contrato where id_contrato='$id_contrato'  LIMIT 1 offset 0 ");
                ...
                $this->SetFont('Arial','',9);
                $this->MultiCell(125,6,"$nombre_mun, $nombre_zona,  $nombre_sector $urbanizacion,  $nombre_calle $edificio,  $numero_casa ","0","L");
...
$pdf->Cuerpo($acceso,$id_orden,$id_gt);
//dl_file($cad);
