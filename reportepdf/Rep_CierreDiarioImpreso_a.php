				...
				$this->Ln();
					...
					$id_franq1=trim($dato[$k]["id_franq"]);
							...
							if($id_f==$id_franq1 || $id_f==$id_franq || $id_f=='0'){
								...
								/*
								echo "<br><br> SELECT  sum(monto_pago) as monto_pago FROM pagos,caja_cobrador,caja ,vista_contrato_dir where
								pagos.id_caja_cob=caja_cobrador.id_caja_cob and 
								caja_cobrador.id_caja=caja.id_caja and 
								pagos.id_contrato=vista_contrato_dir.id_contrato and 
								caja.id_franq='$id_franq'  and 
								fecha_pago='$fecha' and 
								status_pago='PAGADO' and 
								vista_contrato_dir.id_franq='$id_franq1';   $monto_pago_AMC:
								<br>
								
								
								<br> SELECT sum(monto_tp) as monto_tp, count(*) as cant FROM vista_tipopago,vista_contrato_dir where  fecha_pago='$fecha' and id_tipo_pago='TPA00001' and status_pago='PAGADO'  vista_tipopago.id_franq='$id_franq' and  vista_tipopago.id_contrato=vista_contrato_dir.id_contrato  and 
								vista_contrato_dir.id_franq='$id_franq1' ";
								*/
...
$pdf->SetAutoPageBreak(true,10);
		...
		while($row=row($cable)){
			...
			$pdf->Output("../archivos/$id_f/$fecha cierre_completo.pdf",'F');
		}
			...
			$pdf->Output("../archivos/general/$fecha cierre_completo.pdf",'F');
		
			header('Location: informe_comercial_a.php');
