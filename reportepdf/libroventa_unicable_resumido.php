...
IVA",1,'C');
	...
	function Cuerpo($acceso,$desde,$hasta,$consult,$id_est)
	{
		...
		$cable5->objeto->ejecutarSql("SELECT fecha_pago, sum(monto_pago) as monto_pago FROM vista_pago_cont,estacion_trabajo where vista_pago_cont.id_est=estacion_trabajo.id_est $estacion AND  fecha_pago between '$desde' and '$hasta' $consult   group by fecha_pago order by fecha_pago");
		...
		$acceso->objeto->ejecutarSql("SELECT nro_factura,imp_fiscal FROM vista_pago_cont,estacion_trabajo where vista_pago_cont.id_est=estacion_trabajo.id_est $estacion AND  fecha_pago = '$fecha_pago' $consult order by nro_factura LIMIT 1");
		$row=row($acceso);
	...
	//	$fecha_pago=trim($row["monto_iva"]);
		...
		$fill=0;
			...
			$this->SetFont('Arial','',6);
				...
				$this->Cell($w[2],4,"","0",0,"J",$fill);
				$this->Cell($w[3],4,"CLIENTES VARIOS","0",0,"J",$fill);
				//if(trim($row["n_credito"])==''){
					$this->Cell($w[4],4,$nro_factura_ini." : ".$nro_factura_fin,"0",0,"J",$fill);
			/*	}
				else{
					$this->Cell($w[4],4,'',"0",0,"J",$fill);
				}
			*/	
				$this->Cell($w[5],4,$imp_fiscal,"0",0,"J",$fill);
				$this->Cell($w[6],4,"01-Reg","0",0,"C",$fill);
				$this->Cell($w[7],4,utf8_decode(trim($row["cont"])),"0",0,"J",$fill);
				
				if(trim($row["n_credito"])!=''){
					$this->Cell($w[8],4,$nro_factura_ini,"0",0,"J",$fill);
					$this->Cell($w[9],4,$nro_factura_fin,"0",0,"J",$fill);
				}
				else{
				...
				if(trim($row['tipo_cliente'])!="JURIDICO"){
					if($iva<=0){
						if($status_pago=="PAGADO"){
							$this->Cell($w[10],4,number_format($base_imp+0, 2, ',', '.'),"0",0,"R",$fill);
							$total_dia['venta_no_g']+=$base_imp+0;
						}
						else{
							$this->Cell($w[10],4,"-".number_format($base_imp+0, 2, ',', '.'),"0",0,"R",$fill);
						}
						...
						if($status_pago=="PAGADO"){
							$this->Cell($w[11],4,number_format($base_imp+0, 2, ',', '.'),"0",0,"R",$fill);
							$total_dia['venta_no_g_base']+=$base_imp+0;
					...
					if($iva<=0){
						if($status_pago=="PAGADO"){
							$this->Cell($w[15],4,number_format($base_imp+0, 2, ',', '.'),"0",0,"R",$fill);
							$total_dia['venta_no_g_emp']+=$base_imp+0;
						}
						else{
							$this->Cell($w[15],4,"-".number_format($base_imp+0, 2, ',', '.'),"0",0,"R",$fill);
						}
						...
						$this->Cell($w[15],4,'0.00',"0",0,"R",$fill);
						if($status_pago=="PAGADO"){
							$this->Cell($w[16],4,number_format($base_imp+0, 2, ',', '.'),"0",0,"R",$fill);
							$total_dia['venta_no_g_emp_base']+=$base_imp+0;
...
*/
			...
			/*
				...
				*/
				...
				$acceso->objeto->ejecutarSql("SELECT sum(monto_pago) as monto_pago FROM vista_pago_cont where fecha_pago between '$desde' and '$hasta' and status_pago='PAGADO' $estacion1 $consult");
				$row=row($acceso);
				$monto_pago=trim($row["monto_pago"])+0;
				
				$acceso->objeto->ejecutarSql("SELECT sum(base_imp) as venta_no_g_base FROM vista_pago_cont where fecha_pago between '$desde' and '$hasta' and tipo_cliente<>'JURIDICO' and status_pago='PAGADO' $estacion1 $consult");
				$row=row($acceso);
				$venta_no_g_base=trim($row["venta_no_g_base"])+0;
				$venta_no_g=0;
				
				$acceso->objeto->ejecutarSql("SELECT sum(monto_iva) as venta_no_g_iva FROM vista_pago_cont where fecha_pago between '$desde' and '$hasta' and tipo_cliente<>'JURIDICO' and status_pago='PAGADO' $estacion1 $consult");
				$row=row($acceso);
				$venta_no_g_iva=trim($row["venta_no_g_iva"])+0;
				
				$acceso->objeto->ejecutarSql("SELECT sum(monto_reten) as monto_reten FROM vista_pago_cont where fecha_pago between '$desde' and '$hasta'  and status_pago='PAGADO' $estacion1 $consult");
				$row=row($acceso);
				$monto_reten=trim($row["monto_reten"])+0;
				
				$acceso->objeto->ejecutarSql("SELECT sum(islr) as islr FROM vista_pago_cont where fecha_pago between '$desde' and '$hasta'  and status_pago='PAGADO' $estacion1 $consult");
				$row=row($acceso);
				$islr=trim($row["islr"])+0;
				
				
				$acceso->objeto->ejecutarSql("SELECT sum(base_imp) as venta_no_g_emp_base FROM vista_pago_cont where fecha_pago between '$desde' and '$hasta' and tipo_cliente='JURIDICO' and status_pago='PAGADO' $estacion1 $consult");
				$row=row($acceso);
				$venta_no_g_emp_base=trim($row["venta_no_g_emp_base"])+0;
				
				$acceso->objeto->ejecutarSql("SELECT sum(monto_iva) as venta_no_g_emp_iva FROM vista_pago_cont where fecha_pago between '$desde' and '$hasta' and tipo_cliente='JURIDICO' and status_pago='PAGADO' $estacion1 $consult");
				$row=row($acceso);
