...
class PDF extends FPDF
{
	...
	function Titulo($titulo,$id_orden,$nombre_franq)
	{
		...
		$this->MultiCell(100,6,strtoupper(_("   orden de servicio nro")).". "."    $id_orden",'1','L');
	...
	function Cuerpo($acceso,$id_orden)
	{
		...
		$acceso->objeto->ejecutarSql("SELECT  FROM contrato where id_contrato='$id_contrato'  LIMIT 1 offset 0 ");
        ...
        $this->SetFont('Arial','B',10);
        $this->Cell(85,6,"DATOS DEL SERVICIO",'0',0,'L'); 


		$this->Ln();
		$this->SetX($x_izq);
		$this->SetFont('Arial','B',9);
		$this->Cell(32,6,strtoupper(_('tipo de servicio')).":","LTB",0,"J");
		$this->SetFont('Arial','',9);
		
		$this->Cell(58,6,$nombre_tipo_orden,"RTB",0,"J");
		
		$this->SetFont('Arial','B',9);
		$this->Cell(30,6,strtoupper(_('detalle orden')).": ","LTB",0,"J");
		
		$this->SetFont('Arial','',9);
		$this->Cell(70,6,$nombre_det_orden,"RTB",0,"J");
		...
		$this->SetFont('Arial','',9);
		$this->Cell(160,6,''.$nombre_grupo,"RTB",0,"J");
		
		$this->Ln();
		$this->SetFont('Arial','',9);
		$y=$this->GetY();
		$this->SetX($x_izq+30);
		$this->MultiCell(160,6,$detalle_orden,"0","J");

		$this->SetY($y);
		$this->SetX($x_izq);
		$this->SetFont('Arial','B',9);
		$this->Cell(190,6,strtoupper(_('observacion')).": ","LTR",0,"J");
		$this->Ln();
		$this->SetX($x_izq);
	...
	//	$this->SetXY($x_izq,70);
		...
		$this->SetFont('Arial','',9);
		$this->Cell(60,5,$atendido_por,"RTB",0,"L");
		$this->SetFont('Arial','B',9);
		...
		$this->SetFont('Arial','',9);
		$this->Cell(35,5,$hora_emi,"RTB",0,"L");
		...
		$this->SetFont('Arial','',9);
		$this->Cell(60,5,$impreso_por,"RTB",0,"L");
		$this->SetFont('Arial','B',9);
		$this->Cell(15,5,strtoupper(_('fecha')).": ","LTB",0,"L");
		$this->SetFont('Arial','',9);
		...
		$this->Cell(15,5,strtoupper(_('hora')).": ","LTB",0,"L");
		$this->SetFont('Arial','',9);
		$this->Cell(35,5,$hora_imp,"RTB",0,"L");
                ...
                $this->Ln();
                $this->SetX($x_izq+100);
                $this->Cell(110,7,strtoupper(_('Ayudante')).":           ".' ________________________________________',"0",0,"J");
                $this->Ln();
                $this->SetX($x_izq+100);
	...
	function mas_datos($acceso,$id_orden)
	{
		...
		$this->Ln();
		$this->SetX($x_izq);
		$this->Cell(120,10,"CEDULA Y NOMBRE: ","1",0,"J");
		$this->Cell(70,10,"TELEFONO:","1",0,"J");
