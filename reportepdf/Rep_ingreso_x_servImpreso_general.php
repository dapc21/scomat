...
class PDF extends FPDF
{
	...
	//	$w=$this->TituloCampos();
		...
		*/
			...
			$this->SetFont('Arial','B',8);
			$this->SetX(10);
			$w=array(20,20,20,20,20,30,15,20,20,25,18,18,20);
		$header=array(strtoupper(_('fecha')),strtoupper(_('BI G')),strtoupper(_('IVA G')),strtoupper(_('NC. BI G')),strtoupper(_('NC IVA G')),strtoupper(_('Total Nota Credito')),strtoupper(_('Total IVA')),strtoupper(_('TOTAL GENERAL')));
		$this->SetFont('Arial','B',9);
		$this->SetX(10);
		for($k=0;$k<count($header);$k++)
			$this->Cell($w[$k],7,$header[$k],1,0,'C',1);
	...
	while(comparaFecha($desde,$hasta)<=0){
		...
		$acceso->objeto->ejecutarSql("SELECT count(*) cant, sum(base_imp + desc_pago ) as base_imp, sum(monto_iva) as monto_iva, sum(monto_pago) as monto_pago FROM vista_pago_ser where  fecha_pago between '$desde' and '$hast'  and status_pago='PAGADO' $consult");
		$row=row($acceso);
