...
class PDF extends FPDF
{
	...
	function TituloCampos()
	{
		...
		$w=array(10,20,55,18,30,20,20,20);
		$header=array(strtoupper(_('nro')),strtoupper(_('Abonado')),strtoupper(_('nombre y apellido')),strtoupper(_('DEUDA')),strtoupper(_('status')),'POSTE','PRECINTO','P. Adic.');
		$this->SetFont('Arial','B',8);
		$this->SetX(10);
	...
	function Cuerpo($acceso,$where,$deuda,$desde,$hasta,$titulo_list,$salto_p,$cobrador)
	{
		...
		//	ordenDeCorte($acceso,$id_contrato,$tecnico);
			...
			$this->SetFont('Arial','',8);
			$this->Cell($w[2],5,substr(utf8_decode(trim($dato[$i]["nombre"])." ".trim($dato[$i]["apellido"])),0,25),"1",0,"J",$fill);
			$this->SetFont('Arial','',9);
			$this->Cell($w[3],5,$deuda,"1",0,"J",$fill);
			$this->SetFont('Arial','',8);
			$this->Cell($w[4],5,trim($dato[$i]["status_contrato"]),"1",0,"R",$fill);
			$this->Cell($w[5],5,$postel,"1",0,"R",$fill);
			$this->Cell($w[6],5,$etiqueta,"1",0,"R",$fill);
			$this->Cell($w[7],5,$pto,"1",0,"R",$fill);
			
			
			if($numero_casa!=''){
				$numero_casa=" NRO CASA: $numero_casa ; ";
			}
			...
			//ECHO "<BR>:$postel:";
			$dir ="CALLE: $nombre_calle $urbanizacion $edificio ; $numero_casa REF. $direc_adicional";
			$this->Ln();
