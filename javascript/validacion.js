...
var pRif = "";
 ...
 pPrompt = "Error" +": ";
 mMessage = "Error: no puede dejar este espacio vacio";
 pAlphanumeric = "ingrese un texto que contenga solo letras y/o numeros";
 pAlphabetic   = "ingrese un texto que contenga solo letras";
 pTexto   = "ingrese un texto que contenga solo letras numeros";
 pInteger = "ingrese un numero entero";
 pNumber = "ingrese un numero";
 pPhoneNumber = "ingrese un numero de telefono de 11 digitos";
 pEmail = "ingrese una direccion de correo electronico valida";
 pName = "ingrese un texto que contenga solo letras y/o espacios";
 pCedula = "ingrese un numero de cedula valido de 6 hasta 10 Digitos";
 pCedulaE = "ingrese un numero de cedula valido de 6 hasta 10 Digitos";
 pPassword = "la contrasena debe ser minimo 6 digitos";
 pSelect = "no ha Seleccionado ninguna opcion en el Select";
 pDate = "debe introducir una fecha en este formato  DD/MM/AAAA";
 pRif = "ingrese un numero de rif en este formato xxxxxxxx-x";
...
function valdate(fecha){
	if(fecha!='')
	{
		...
		if(date.length==3)
		{
			if(date[2].length!=4 || date[1].length!=2 || date[0].length!=2)
			{
				alerta('Error, debe introducir una fecha en este formato  DD/MM/AAAA');
				return false;
			}
			else{
				return true;
		...
		alerta('Error, debe introducir una fecha valida en este formato  DD/MM/AAAA');
			return false;
	}
}
...
{   
    ...
    if ((campo.value == null) || (campo.value.length == 0)){
		if(vacio==true){
	        ...
	        alerta(mMessage+" : "+campo.name);	campo.select();						
		   return false;
		}
	}		
    if (funcion(campo.value) == true){  return true;}
    else{				
		alerta(pPrompt + msg); campo.select(); return false;
	}
}
...
{   
    ...
    if ((document.getElementById(campo).value == null) || (document.getElementById(campo).value.length == 0)){
		if(vacio==true){
	        ...
	        alerta(mMessage+" : "+document.getElementById(campo).name);	document.getElementById(campo).select();						
		   return false;
		}
	}		
    if (funcion(document.getElementById(campo).value) == true){  return true;}
    else{				
		alerta(pPrompt + msg); document.getElementById(campo).select(); return false;
	}
	*/
}
...
function soloNumerico(e){ // 1
	//alerta(document.f1.monto1.value);
...
function aplicaDesc(e){ // 1
	...
	if(validaCampo(document.f1.monto1,isNumber)){
		...
		if(parseFloat(document.f1.monto1.value)<parseFloat(document.f1.monto.value)){
			...
			alerta("Error, El descuento debe ser menor al costo del cargo");
		}
	}
}
	...
	if(validaCampo(document.f1.monto_dc,isNumber)){
		...
		if(verRadiotipo_nota()=="NOTA CREDITO"){
			if(parseFloat(document.f1.monto_dc.value)>parseFloat(document.f1.monto_resta.value)){
				...
				alerta("Error, El monto de la nota de credito debe ser menor al costo del cargo");
				return;
			}
