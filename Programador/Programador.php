...
if($_SESSION["autenticacion"]!="On"){
	if($valor[0]=='Configuracion')
		...
		$valor[1]=strtolower(trim($valor[1]));
	...
	if(clase($valor[1]))
	{
	 ...
	 switch($clase)
	 {
		...
		case crearClase:
			...
			crearDataGrid($valor);
			crearJavaScript($valor);
			agregarIndex($valor);
			//crearClaseValidacion($valor);
			echo crearClase($valor,$acceso);
			break;
		case finalizarmodulo:
			finalizardatataGrid($valor);
			echo finalizarClase($valor);
			break;
		case modificarClase:
...
function crearClase($cadena,$acceso)
{
	...
	$acceso->objeto->ejecutarSql("select *from modulo  where (codigomodulo ILIKE 'SVCH%') ORDER BY codigomodulo desc"); 
	$codigomodulo = "SVCH".verCodigo($acceso,"codigomodulo");

	$acceso->objeto->ejecutarSql("INSERT INTO modulo (codigomodulo, nombremodulo, descripcionmodulo, statusmodulo, namemodulo) VALUES ('$codigomodulo', '$modulo', '$modulo', 'ACTIVO', '$modulo');");
	$acceso->objeto->ejecutarSql("INSERT INTO modulo_perfil (codigoperfil,codigomodulo, incluir, modificar, eliminar) VALUES ('PERF001','$codigomodulo', 'TRUE', 'TRUE', 'TRUE');");
	

	return  'true';
}
function crearDataGrid($cadena)
{
	$fp = fopen("defaultDataGrid.php","r");		
	$id = fopen("../procesos/datagrid_".$cadena[1].".php","a+");		
	while($linea= fgets($fp))
	{	
		fwrite($id,str_replace("Clase",strtolower($cadena[1]),$linea));
	}	
	fclose ($id);
	fclose ($fp);
	return  'true';
}
function crearJavaScript($cadena)
{
	$fp = fopen("defaultjs.js","r");		
	$id = fopen("../js/".$cadena[1].".js","a+");		
	while($linea= fgets($fp))
	{
		fwrite($id,str_replace("-Clase-",strtolower($cadena[1]),$linea));
	}	
	...
	if (file_exists("../procesos/datagrid_".$cadena[1].".php"))
	{
		...
		{	
			if(strstr($linea,'dato",')){
				$linea=str_replace('dato",',$campoClave.',dato",',$linea);
			}
			if(strstr($linea,"$x->setColumnHeader('dato', 'dato');")){
				fwrite($id,'$x->setColumnHeader(\''.$cadena[3].'\', \''.$cadena[4].'\');');
				fwrite($id,'
');
			}


			$linea=str_replace("campoClave",strtolower($cadena[3]),$linea);
...
function finalizardatataGrid($cadena)
{
		$campoClave=trim($cadena[3]);
	if (file_exists("../procesos/datagrid_".$cadena[1].".php"))
	{
		...
		{	
			if(strstr($linea,'dato",')){
				$linea=str_replace(',dato",',$campoClave.'",',$linea);
			}
			fwrite($id,$linea);
		}	
		fclose ($id);
...
function agregarCampoJS($cadena,$tipo)
{
		$campoClave=trim($cadena[3]);
	if (file_exists("../js/".$cadena[1].".js"))
	{
		copy("../js/".$cadena[1].".js","copia");		
		$fp = fopen("copia","r");		
		$id = fopen("../js/".$cadena[1].".js","w+");
		while ($linea= fgets($fp))
		{	
			if($tipo=='campoClave'){
				$linea=str_replace("campoClave",strtolower($cadena[3]),$linea);
				...
				if(strstr($linea,'"dato":getD("dato")')){
					if($cadena[2]=='radio'){
				fwrite($id,'                  "'.$cadena[3].'":getOption("'.$cadena[3].'"),
');
					}
					else{
						fwrite($id,'                  "'.$cadena[3].'":getD("'.$cadena[3].'"),
');
					}
				}
				else if(strstr($linea,'setD("dato",campo.dato);')){
					if($cadena[2]=='radio'){
						fwrite($id,'               setOption("'.$cadena[3].'",campo.'.$cadena[3].');
');		
					}else{
						fwrite($id,'               setD("'.$cadena[3].'",campo.'.$cadena[3].');
');		
					}
...
function crearClaseControladora($cadena)
{
	...
	{	
		...
		if(strstr($linea,'function validarDato(){')){
			fwrite($id,'function validar'.$cadena[1].'(){ conexionPHP("validarExistencia.php","1=@'.strtolower($cadena[1]).'","dato=@"+data());}
');
			$x=true;
		}
...
{
	$fp = fopen("defaultFormulario.php","r");		
	$id = fopen("../Formulario/".$cadena[1].".php","a+");		
	while($linea= fgets($fp))
	{	
		fwrite($id,str_replace("Clase",$cadena[1],$linea));
	}	
...
function agregarIndex($cadena)
{
	...
	while ($linea=fgets($fp))
	{
		if(strstr($linea,'<!--<script language="JavaScript" type="text/javascript" src="js/-Clase-.js"></script>-->')){
			fwrite($id,'		<script language="JavaScript" type="text/javascript" src="js/'.$cadena[1].'.js"></script>
');
		}
		else if(strstr($linea,'<!--<li><a  href="javascript:cargar_form_-Clase-();">-Clase-</a></li>-->')){
			fwrite($id,'							<li><a  href="javascript:cargar_form_'.$cadena[1].'();">'.$cadena[1].'</a></li>
');
		}
		fwrite($id,$linea);
...
function agregarCampoFormulario($cadena,$tipo)
{
	$x=false;
	$clase=trim($cadena[1]);
	copy("../Formulario/".$cadena[1].".php","copia");		
	$fp = fopen("copia","r");		
	...
	while ($linea=fgets($fp))
	{
				...
				if($tipo=='campoClave'){
					if(strstr($linea,'<input  type="hidden" value="dato" name="dato" id="dato">')){
						fwrite($id,'				<input  type="hidden" name="'.$cadena[3].'" id="'.$cadena[3].'" maxlength="'.$cadena[8].'" size="'.$cadena[7].'"  value="<?php $acceso->objeto->ejecutarSql("select *from '.strtolower($cadena[1]).'  where ('.$cadena[3].' ILIKE \'$ini_u%\') ORDER BY '.$cadena[3].' desc"); echo $ini_u.verCoo($acceso,"'.$cadena[3].'")?>">
');
					}
				}
			if($tipo=='campo'){
				$validacion='';
				if($cadena[6]=='isInteger'){
					$validacion=' required="" data-parsley-type="integer"';
				} else if($cadena[6]=='isAlphanumeric'){
					$validacion='required=""  data-parsley-type="alphanum"';
				} else if($cadena[6]=='isNumber'){
					$validacion='required="" data-parsley-type="number"';
				} else if($cadena[6]=='isEmail'){
					$validacion='required="" data-parsley-type="email"';
				} else if($cadena[6]=='isPhoneNumber'){
					$validacion='required="" data-parsley-length="[11, 11]" data-parsley-type="integer"';
				} else if($cadena[6]=='isCedula'){
					$validacion='required="" data-parsley-length="[6, 10]" data-parsley-type="integer"';
				}
				

				if(strstr($linea,'<input  type="hidden" value="dato" name="dato" id="dato">')){
					if($cadena[2]=='texto'){
						fwrite($id,'				<div class="form-group col-lg-3 col-md-6 col-sm-6 col-xs-12">
					<label>'.$cadena[4].'</label>
					<input data-parsley-id="'.$cadena[3].'" required=""  '.$validacion.'  class="form-control" type="text" name="'.$cadena[3].'" id="'.$cadena[3].'" maxlength="'.$cadena[11].'"  value="" >
					<ul id="parsley-id-'.$cadena[3].'" class="parsley-errors-list"></ul>							
				</div>
...
');
						...
						fwrite($id,'					<div class="form-group col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<label>'.$cadena[4].'</label>
						<textarea data-parsley-id="'.$cadena[3].'" required=""  '.$validacion.'  data-parsley-type="alphanum" class="form-control" name="'.$cadena[3].'" id="'.$cadena[3].'"  rows="'.$cadena[11].'"></textarea>
						<ul id="parsley-id-'.$cadena[3].'" class="parsley-errors-list"></ul>
				</div>
...
');
						...
						$cade='					<div class="form-group col-lg-3 col-md-6 col-sm-6 col-xs-12">
						<label>'.$cadena[4].'</label>
						<select data-parsley-id="'.$cadena[3].'" required=""  '.$validacion.' class="form-control" name="'.$cadena[3].'" id="'.$cadena[3].'" onchange="">';
					$cade=$cade.'
							...
							for($i=0;$i<count($valor);$i++)
							{
								$cad=explode(",",$valor[$i]);
								$cade=$cade.'
							<option value="'.trim($cad[0]).'">'.trim($cad[1]).'</option>';
							}
...
';
						...
						$valor=explode(";",$cadena[9]);
							...
							$cade='					<div class="form-group col-lg-6 col-md-12 col-sm-12 col-xs-12">
						<label>Status</label>
						<div>
							<input  type="radio" name="'.$cadena[3].'" value="ACTIVO"CHECKED>ACTIVO';
							
							for($i=1;$i<count($valor);$i++)
							{
								
								$cade=$cade.'
							&nbsp;&nbsp;&nbsp;<input  type="radio" name="'.$cadena[3].'" value="'.strtoupper(trim($valor[$i])).'">'.strtoupper(trim($valor[$i])).'';
							}
...
function agregarCampoControlador($cadena,$tipo)
{
	...
	while ($linea=fgets($fp))
	{
		...
		if(strstr($linea,'function validar'.$cadena[1].'(){')){
				if($tipo=='campoClave'){
					fwrite($id,'function validar'.$cadena[1].'(){ conexionPHP("validarExistencia.php","1=@'.strtolower($cadena[1]).'","'.$cadena[3].'=@"+'.$cadena[3].'());}
');
					$y=false;
				}
		}
...
');
		...
		if($x==true)
		{
			if(strstr($linea,'confirmacion(tipoDato,clase'))
			{
			  if($cadena[2]=='radio'){
...
');
		...
		if(strstr($linea,'return $acceso->objeto->ejecutarSql("insert')){		
		...
		else if(strstr($linea,'$this->dato = ')){
			$linea = str_replace('$this->dato = $dat[\'dato\'];','$this->datos = $dat[\'datos\'];
		$this->dato = $dat[\'dato\'];',$linea);
			fwrite($id,str_replace("datos",$cadena[3],$linea));
		}
		else{
...
function finalizarClase($cadena)
{
	copy("../Clases/".$cadena[1].".php","copia");		
	$fp = fopen("copia","r");		
	$id = fopen("../Clases/".$cadena[1].".php","w+");		
